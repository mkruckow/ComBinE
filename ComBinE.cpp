// Kruckow & Tauris 2013-2018. version 1.2.0
...
#include <sstream>	//in-/output to/from strings
...
const double ignorev=-1.0e+99;	//value to ignore a variable

const int version0=1;		//code version
const int version1=2;		//code version
const int version2=0;		//code version
...
double alphaCE=0.5+0.5;		// 50% efficiency of converting E_orb to E_kin env. during CE
...
double alphaTH=0.5+0.5;		// factor to interpolate between lambda with/without thermal and radiation energy (0.0-1.0: interpolate, -1: take old lambdatable, else assume viral equilibrium)
char output='M';		// output format: T (tiny) or M (massive)
double binfrac=1.0;		// binary fraction, how many initaial binaries vs initial single stars
...
int main(int argc, char *argv[])	//argc: count of arguments, argv: argument values
{
  ...
  ifstream systemsin;		// enable reading systems from file
  ofstream binaryout;		// enable writing to file the output of the binary system
  ofstream primaryout;		// enable writing to file the output of the primary
  ofstream secondaryout;	// enable writing to file the output of the secondary
  bool singledebug=false;	// additional output for debuging in single run
  bool wanted=false;		// search for a wanted system
  bool initialfromfile=false;	// whether to read initial values from file
  string systeminline;		// a line
  istringstream ssysteminline;	// stream of the line
// SET PHYSICS PARAMETERS:
  ...
  double Mp=-1.0, Ms=-1.0, a=-1.0;	// primary mass, secondary mass, semi-major-axis of one system
  ...
  double maxa=a_min;		// maximum semi-major axis during the evolution in Rsun
...
/*  double age, t_thermal;	//old code
  ...
  long seeds[8]={-8809843958428921796,-4468887128648617210,-5430439732881999393,6759374438094590665,0,0,0,0};	//current seeds of the random number generators{0,0,0,0,0,0,0}
  long discard[8]={0,0,0,0,0,0,0,0};	//number of random numbers to discard{0,0,0,0,0,0,0}
  ...
  static long discard_rotation=0;	//discards of rotation generator
//  long saveseeds[7]={0,0,0,0,0,0,0,0};	//old values of seeds to check if same system are calculated again
//  static long discardout[7]={discard[0],discard[1],discard[2],discard[3],discard[4],discard[5],discard[6],discard[7]};	//for output discard
...
/*  long* saveseed=(long *)malloc(sizeof(long));	//old values of seed to check if same system are calculated again
  ...
  long cofID=0;			//number of systems written to commonoutputformat
  long cofLINES=0;		//number of data lines written to commonoutputformat
  int cofcont=1;		//whether continueing with the output (contains the last written event)
  time_t t1, t2;		//times
...
/*      input.ignore(10000,':');	//ignore the characters until ':', here "ADDRAN:"
      ...
      input.ignore(10000,':');	//ignore the characters until ':', here //"TIDES (=0 NO TIDES; =1 SYNCHRONISING TIDES):"
      input >> tides;		//read in the flag of tides
      input.ignore(10000,':');	//ignore the characters until ':', here "OUTPUT FORMAT (=M MASSIVE OUTPUT; =T TINY OUTPUT; =C COMMON OUTPUT FORMAT):"
      ...
      input.ignore(10000,':');  //ignore the characters until ':', here "INITIAL BINARY FRACTION (%):"
      input >> binfrac;		//read the binary fraction (in %)
      ...
      input.ignore(10000,':');	//ignore the characters until ':', here "INITIAL ROTATIONAL DISTRIBUTION (=-1 SYNCHRONISED; =0 NON-ROTATING):"
      ...
      binfrac /= 100.0;   	//convert the per cent input
      if ((output!='M')&&(output!='T')&&(output!='C')){
...
/*      Mp = Mp_max;		//set primary mass in Msun
      ...
      a = a_max;		//set semi-major-axis in Rsun	//cbrt(square((12.0/365.25)/(2.0*M_PI))*(G*(Mp_max+Ms_max)));
...
//      seed = 13579;		//set specific seed
      if (argc>2){
        ...
        if (argc>3){
          ...
          if (Ms == 0.0) Ms = 0.01*accuracy;
          if (argc>4){
            a = atof(argv[4]);	//read in semi-major-axis in Rsun from input arguments
            if (argc>6){
              vp = atof(argv[5])*1.0e3/Rsun*yr;	//min(fabs(atof(argv[5]))*1.0,100.0)/100.0 *20000.0*pow(Mp,-0.55); //omegap/s are no angular velocities!
              vs = atof(argv[6])*1.0e3/Rsun*yr; //min(fabs(atof(argv[6]))*1.0,100.0)/100.0 *20000.0*pow(Ms,-0.55);
              if (argc>7){
                seed = atol(argv[7]);	//read in random seed from input arguments
                if (seed<0) nseed = true;	//seed was negative before
                else seed = -seed;	//switch seed to negative value;
                if (argc>8){
                  kickv = atof(argv[8]);	//read in kickvelocity in km/s(<0 is random) from input arguments
                }
              }
            }else{
              vp = ignorev;
  ...
  if (Zdist==0){
    metal = 0.0088;	//set metallicity: Milky Way metallicity(~0.009)
  }else if (Zdist==-1){
    metal = 0.0047;		//set metallicity: LMC(~0.005)
  }else if (Zdist==-2){
    metal = 0.0021;		//set metallicity: SMC(~0.002)
  }else if (Zdist==-3){
  ...
  if ((tides==0)&&(single)&&(vp+vs>accuracy)) tides = 1;	//calculate tides for rotating stars
  ...
  readtables(metal, false);	//initializes and read in all tables	//vp>0
...
//      if ((stararray[i].TAMS>2)&&(stararray[i].n-1>stararray[i].TAMS)) cerr << "[" << i << "]: n=" << stararray[i].n << " m[TAMS]" << stararray[i].m[stararray[i].TAMS] << " t[TAMS]" << stararray[i].t[stararray[i].TAMS] << " r[TAMS]" << stararray[i].r[stararray[i].TAMS] << " cm[TAMS]" << stararray[i].cm[stararray[i].TAMS] << " llum[TAMS]" << stararray[i].llum[stararray[i].TAMS] << " lteff[TAMS]" << stararray[i].lteff[stararray[i].TAMS] << " lambda[TAMS]" << stararray[i].lambda[stararray[i].TAMS] << " cc[TAMS]" << stararray[i].cc[stararray[i].TAMS] << << " cf[TAMS]" << stararray[i].cf[stararray[i].TAMS] << endl;	//write out track point at TAMS
    ...
    if (stararray2!=stararray){
      ...
      for(i=0;i<nstar2;i++){		//go through all masses in stararray2
        ...
        cerr << "[" << i << "]: n=" << stararray2[i].n << " m[0]" << stararray2[i].m[0] << " t[0]" << stararray2[i].t[0] << " r[0]" << stararray2[i].r[0] << " cm[0]" << stararray2[i].cm[0] << " llum[0]" << stararray2[i].llum[0] << " lteff[0]" << stararray2[i].lteff[0] << " lambda[0]" << stararray2[i].lambda[0] << " cc[0]" << stararray2[i].cc[0] << " cf[0]" << stararray2[i].cf[0] << endl;	//write out maximum values
        for(j=1;j<min(stararray2[i].n-1,999);j++){
          cerr << "[" << i << "]: j=" << j << " m[j]" << stararray2[i].m[j] << " t[j]" << stararray2[i].t[j] << " r[j]" << stararray2[i].r[j] << " cm[j]" << stararray2[i].cm[j] << " llum[j]" << stararray2[i].llum[j] << " lteff[j]" << stararray2[i].lteff[j] << " lambda[j]" << stararray2[i].lambda[j] << " cc[j]" << stararray2[i].cc[j] << " cf[j]" << stararray2[i].cf[j] << endl;	//write out track point
        }
//        if ((stararray2[i].TAMS>2)&&(stararray2[i].n-1>stararray2[i].TAMS)) cerr << "[" << i << "]: n=" << stararray2[i].n << " m[TAMS]" << stararray2[i].m[stararray2[i].TAMS] << " t[TAMS]" << stararray2[i].t[stararray2[i].TAMS] << " r[TAMS]" << stararray2[i].r[stararray2[i].TAMS] << " cm[TAMS]" << stararray2[i].cm[stararray2[i].TAMS] << " llum[TAMS]" << stararray2[i].llum[stararray2[i].TAMS] << " lteff[TAMS]" << stararray2[i].lteff[stararray2[i].TAMS] << " lambda[TAMS]" << stararray2[i].lambda[stararray2[i].TAMS] << " cc[TAMS]" << stararray2[i].cc[stararray2[i].TAMS] << " cf[TAMS]" << stararray2[i].cf[stararray2[i].TAMS] << endl;	//write out track point at TAMS
        cerr << "[" << i << "]: n=" << stararray2[i].n << " m[n-1]" << stararray2[i].m[stararray2[i].n-1] << " t[n-1]" << stararray2[i].t[stararray2[i].n-1] << " r[n-1]" << stararray2[i].r[stararray2[i].n-1] << " cm[n-1]" << stararray2[i].cm[stararray2[i].n-1] << " llum[n-1]" << stararray2[i].llum[stararray2[i].n-1] << " lteff[n-1]" << stararray2[i].lteff[stararray2[i].n-1] << " lambda[n-1]" << stararray2[i].lambda[stararray2[i].n-1] << " cc[n-1]" << stararray2[i].cc[stararray2[i].n-1] << " cf[n-1]" << stararray2[i].cf[stararray2[i].n-1] << endl;		//write out last track point
      }
    }
    cerr << "nhe1 " << nhe1 << endl;
    ...
    for(i=0;i<nhe1;i++){		//go through all masses in hestararray1
      ...
      cerr << "[" << i << "]: n=" << hestararray1[i].n << " m[0]" << hestararray1[i].m[0] << " t[0]" << hestararray1[i].t[0] << " r[0]" << hestararray1[i].r[0] << " cm[0]" << hestararray1[i].cm[0] << " llum[0]" << hestararray1[i].llum[0] << " lteff[0]" << hestararray1[i].lteff[0] << " lambda[0]" << hestararray1[i].lambda[0] << " cc[0]" << hestararray1[i].cc[0] << " cf[0]" << hestararray1[i].cf[0] << endl;	//write out maximum values
      for(j=1;j<min(hestararray1[i].n-1,999);j++){
        cerr << "[" << i << "]: j=" << j << " m[j]" << hestararray1[i].m[j] << " t[j]" << hestararray1[i].t[j] << " r[j]" << hestararray1[i].r[j] << " cm[j]" << hestararray1[i].cm[j] << " llum[j]" << hestararray1[i].llum[j] << " lteff[j]" << hestararray1[i].lteff[j] << " lambda[j]" << hestararray1[i].lambda[j] << " cc[j]" << hestararray1[i].cc[j] << " cf[j]" << hestararray1[i].cf[j] << endl;	//write out track point
      }
//      if ((hestararray1[i].TAMS>2)&&(hestararray1[i].n-1>hestararray1[i].TAMS)) cerr << "[" << i << "]: n=" << hestararray1[i].n << " m[TAMS]" << hestararray1[i].m[hestararray1[i].TAMS] << " t[TAMS]" << hestararray1[i].t[hestararray1[i].TAMS] << " r[TAMS]" << hestararray1[i].r[hestararray1[i].TAMS] << " cm[TAMS]" << hestararray1[i].cm[hestararray1[i].TAMS] << " llum[TAMS]" << hestararray1[i].llum[hestararray1[i].TAMS] << " lteff[TAMS]" << hestararray1[i].lteff[hestararray1[i].TAMS] << " lambda[TAMS]" << hestararray1[i].lambda[hestararray1[i].TAMS] << " cc[TAMS]" << hestararray1[i].cc[hestararray1[i].TAMS] << " cf[TAMS]" << hestararray1[i].cf[hestararray1[i].TAMS] << endl;	//write out track point at TAMS
      cerr << "[" << i << "]: n=" << hestararray1[i].n << " m[n-1]" << hestararray1[i].m[hestararray1[i].n-1] << " t[n-1]" << hestararray1[i].t[hestararray1[i].n-1] << " r[n-1]" << hestararray1[i].r[hestararray1[i].n-1] << " cm[n-1]" << hestararray1[i].cm[hestararray1[i].n-1] << " llum[n-1]" << hestararray1[i].llum[hestararray1[i].n-1] << " lteff[n-1]" << hestararray1[i].lteff[hestararray1[i].n-1] << " lambda[n-1]" << hestararray1[i].lambda[hestararray1[i].n-1] << " cc[n-1]" << hestararray1[i].cc[hestararray1[i].n-1] << " cf[n-1]" << hestararray1[i].cf[hestararray1[i].n-1] << endl;		//write out last track point
    }
    cerr << "nhe " << nhe << endl;
  ...
  if (number==-1){
    initialfromfile = true;
    systemsin.open("systems.in");	//open file with the name "systems.in"
    if (!systemsin.is_open()) cerr << "#Error: can't open systems.in" << endl;
    number = count(istreambuf_iterator<char>(systemsin), istreambuf_iterator<char>(), '\n');
    cout << "read " << number << "lines from systems.in" << endl;
    systemsin.close();
    systemsin.open("systems.in");	//reopen file with the name "systems.in"
  }
  ...
  if (grid){			//run a grid starting at the maximal values
    cout << "Grid: Mp in [" << Mp_min << ", " << Mp_max << "] Msun; Ms in [" << Ms_min << ", " << Ms_max << "] Msun; a in [" << a_min << ", " << a_max << "] Rsun" << endl;
...
//    number = 1000000;		//maximal number of grid points
  ...
  SetSeed(seed,discard,8);	//user interface function to manipulate discard
  ...
  discard_rotation = discard[7];	//discards of rotation generator
  if (debug) cerr << "discard={" << discard_mp << "," << discard_q << "," << discard_a << "," << discard_kick << "," << discard_phase << "," << discard_eccentricity << "," << discard_galactic << "," << discard_rotation << "}" << endl;
  for (i=0; i<8; i++){
...
/*  if (datafile){			//write parameters to data.out
...
#pragma omp parallel for private(system, i, j, t2, n) firstprivate(wanted, Mp, Ms, a, e, metal, vp, vs, x, y, z, rhostar, seed, seeds, discard_mp, discard_q, discard_a, discard_kick, discard_phase, discard_eccentricity, discard_galactic, discard_rotation, phasehistory, datafile, dist, grid, plot) shared(count_mp, count_q, count_a, count_kick, count_phase, count_eccentricity, count_galactic, count_rotation) reduction(+ : cnotrackupdate, cRLO, cRLOA, cRLOB, cRLOBB, cCE, cCEA, cCEB, cCEBB, cMerger, cMergerRLO, cMergerCE, cMergerSN, csupernova, cNS, cBH, cplanetarynebula, cWD, cfinalunknown, ccrashed, cdestroyed, cdmerged, cdexploded, cdunknown, cfinalsingle, csborn, csmerged, csexploded, csunknown, cfinal, cWDWD, cWDNS, cWDBH, cNSWD, cNSNS, cNSBH, cBHWD, cBHNS, cBHBH, cgw, cgwWDWD, cgwWDNS, cgwWDBH, cgwNSWD, cgwNSNS, cgwNSBH, cgwBHWD, cgwBHNS, cgwBHBH)
...
#pragma omp critical
{
    if (((number>99)&&(done%(number/100)==0))||((number>100000000)&&(done%1000000==0))||((grid)&&(done%100000==0))){	//progress: output at each per cent or every million systems
      ...
      if ((output=='M')||(output=='C')){
        timefactor = (difftime(t2,t1)+1.0)/done*(number-done);	//get remaining time
        if (timefactor>90000.0) timefactor=86400.0;	//in days
        else if (timefactor>6000.0) timefactor=3600.0;	//in hours
        else if (timefactor>100.0) timefactor=60.0;	//in minutes
        else timefactor=1.0;				//in seconds
        cerr << " Number: " << done;	//output of number
        if (done>0){
          if (difftime(t2,t1)>1.0) cerr << " speed: " << done/difftime(t2,t1) << "systems/second [" << done/(difftime(t2,t1)+1.0) << "," << done/(difftime(t2,t1)-1.0) << "] remaining: " << difftime(t2,t1)/done*(number-done)/timefactor << "+-" << 1.0/done*(number-done)/timefactor;	//output of speed
          else cerr << " speed: >" << done/(difftime(t2,t1)+1.0) << "systems/second remaining: <" << (difftime(t2,t1)+1.0)/done*(number-done)/timefactor;	//output of speed
          if (timefactor==86400.0) cerr << "days" << endl;	//in days
          else if (timefactor==3600.0) cerr << "hours" << endl;	//in hours
          else if (timefactor==60.0) cerr << "minutes" << endl;	//in minutes
          else  cerr << "seconds" << endl;			//in seconds
        }else{
          cerr << endl;			//in seconds
        }
      }
    }else if (number<100){
...
//      srand(i); 		//set new seed
      if (stepsize<0){		//logarithmic grid
        if (Ms/pow(10.0,-stepsize)>=Ms_min) Ms /= pow(10.0,-stepsize);	//use log-steps per dex in secondary mass
        else{
          if (Mp/pow(10.0,-stepsize)>=Mp_min) Mp /= pow(10.0,-stepsize);	//use log-steps per dex in primary mass
          else{
            if (a/pow(10.0,-stepsize)>=a_min) a /= pow(10.0,-stepsize);	//use log-steps per dex in semi-major-axis
...
//              cout << "grid end: " << i << endl;
          ...
          Ms = fmin(Ms_max,Mp);		//get back to maximum in secondary mass	//Ms = 0.999*Mp;
        }
      }else{		//linear grid
...
//              cout << "grid end: " << i << endl;
    ...
    }else if(initialfromfile){
      //read in the initial parameters
      getline(systemsin,systeminline);
//      cout << systeminline << endl;
      ssysteminline.clear();
      ssysteminline.str(systeminline);
      ssysteminline >> Mp >> Ms >> a >> e;
      a = cbrt(square(a*day/(2.0*M_PI))*(G*(Mp+Ms)));	//convert period in days to semi-major axis in Rsun
      initsystem(system, Mp, Ms/Mp, a, e, 0.0, metal, vp, vs, x, y, z, vx, vy, vz, rhostar);	//create new system with specified parameters (at ZAMS: t=0)
    }else{
...
/*evolve the system*/
    while ((abs(system.phase[system.n-1])<90)&&(system.n<100)){	//evolve the system till the end or the system evolved through more than 100 phases
//***********************
      evolve(system);
//***********************
...
//        cout << "memory check: &savetrack=" << &savetrack << " &system.sec.track=" << &system.sec.track << endl;
//        cout << "&savetrack.(n,m,t,r,cm,llum,lteff,lambda,cc)=\t\t" << &savetrack.n << ",\t" << savetrack.m << ",\t" << savetrack.t << ",\t" << savetrack.r << ",\t" << savetrack.cm << ",\t" << savetrack.llum << ",\t" << savetrack.lteff << ",\t" << savetrack.lambda << ",\t" << savetrack.cc << "\n&system.sec.track.(n,m,t,r,cm,llum,lteff,lambda,cc)=\t" << &system.sec.track.n << ",\t" << system.sec.track.m << ",\t" << system.sec.track.t << ",\t" << system.sec.track.r << ",\t" << system.sec.track.cm << ",\t" << system.sec.track.llum << ",\t" << system.sec.track.lteff << ",\t" << system.sec.track.lambda << ",\t" << system.sec.track.cc << endl;
//        cout << endl << "system.sec.track.n=" << system.sec.track.n << " system.sec.track.(m,t,r,cm,llum,lteff,lambda)=\n[(" << system.sec.track.m[0] << "," << system.sec.track.t[0] << "," << system.sec.track.r[0] << "," << system.sec.track.cm[0] << "," << system.sec.track.llum[0] << "," << system.sec.track.lteff[0] << "," << system.sec.track.lambda[0] << "," << system.sec.track.cc[0] << "," << system.sec.track.cf[0] << ")";
...
//          cout << ", (" << system.sec.track.m[j] << "," << system.sec.track.t[j] << "," << system.sec.track.r[j] << "," << system.sec.track.cm[j] << "," << system.sec.track.llum[j] << "," << system.sec.track.lteff[j] << "," << system.sec.track.lambda[j] << "," << system.sec.track.cc[j] << "," << system.sec.track.cf[j] << ")";
        }
//        cout << "]" << endl;
        // circularization
        system.a[system.n-2] *= 1.0-square(system.e[system.n-2]);	//calculate semi-major axis with angular momentum conservation
...
/*write out system*/
    ...
    if (system.phase[n]==-1000){	//crashed systems
      screen = true;
    }
//    if ((system.prim.stage[n]==4)&&(system.sec.stage[n]==4)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.phase[n-2]%10!=5)) wanted = true;
//    if ((system.prim.stage[n]==4)&&(system.sec.stage[n]==3)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.formation==34)) wanted = true;	//WDNS system
//    if ((system.phase[n-1]==4)&&((system.phase[n-2]==12)||(system.phase[n-2]==21))) wanted = true;	//merger after RLO
//    if ((system.prim.stage[n]==4)&&(system.sec.stage[n]==4)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.prim.stage[n-3]==3)) wanted = true;	//NSNS from WD implosion
//    if ((system.prim.stage[n]==4)&&(system.sec.stage[n]==4)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.prim.m[0]<7)) wanted = true;	//too low mass to get NSNS
//    if ((system.prim.stage[n]==4)&&(system.sec.stage[n]==4)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.sec.m[0]<3.4)) wanted = true;	//too low mass to get NSNS
//    if ((system.prim.stage[n]==4)&&(system.sec.stage[n]==4)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.phase[max(n-1,0)]==25)&&(system.phase[max(n-3,0)]==23)) wanted = true;
//    if ((system.phase[n]>=90)&&(system.phase[n]<=99)&&((system.prim.m[n]>50.0)||(system.sec.m[n]>50.0))) wanted = true;	//very massive BHs
//    if ((system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.prim.stage[n]==5)&&(system.sec.stage[n]==5)&&(system.prim.m[n]<10.0)&&(system.sec.m[n]<10.0)) wanted = true;	//low mass BHs
//    if ((system.formation==45)&&(system.phase[n]>=90)&&(system.phase[n]<=99)) wanted = true;	//NSBH
//    if ((system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.e[n]<=0.10)&&(system.e[n]>=0.08)&&(system.P[n]/day<=11)&&(system.P[n]/day>=9)&&(abs(system.prim.stage[n]-system.sec.stage[n])<=1)&&(system.prim.stage[n]+system.sec.stage[n]<=8)&&(system.prim.stage[n]+system.sec.stage[n]>=7)) wanted = true;	//WDNS system
...
/*    if ((wanted)&&(system.tgw>tmax)){
    ...
    if (snapshots &&(galintegrator!=0)&&(galpotential!=0)){
      if ((system.phase[n]>=90)&&(system.phase[n]<=99)&&((system.formation==34)||(system.formation==43))&&((system.prim.m[n]<0.3)||(system.sec.m[n]<0.3))&&(system.prim.t[n]<1.3e+10)){	// (NS+WD[<0.3Msun])
    ...
    sprintf(phasehistory,"%d",system.phase[0]);	//start phase history
    system.stagechange = 0;	//use stagechange as index for movement calculation
    for (j=1; j<system.n; j++){
      if ((system.phase[j]==4)&&(system.prim.stage[j]>3)&&(system.prim.stage[j]<6)&&(system.sec.stage[j]>3)&&(system.sec.stage[j]<6)) screen = true;
      if (j<n) sprintf(phasehistory,"%.189s%d",phasehistory,system.phase[j]);	//add next phase to phase history
    //write to distributionout # open/close file in statistics
/*      if ((distributionout!=NULL)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.prim.stage[j]>3)&&(system.sec.stage[j]>3)&&((system.prim.stage[j-1]<3)||(system.sec.stage[j-1]<3))){	//write specified system (NS/BS binaries) to distribution output
...
/*      if ((distributionout!=NULL)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&& snapshots && galacticintegration){	//write snapshot systems to distribution output
...
/*      if ((distributionout!=NULL)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&((system.formation==34)||(system.formation==44))){	//write specified system to distribution output
...
/*      if ((distributionout!=NULL)&&(((system.prim.stage[j]==3)&&(system.prim.m[j]>1.35)&&(system.phase[j]==21))||((system.sec.stage[j]==3)&&(system.sec.m[j]>1.35)&&(system.phase[j]==12)))){	//write specified system WD exceeding maximum mass -> SN 1a to distribution output
        fprintf(distributionout,"%12.6e\t%9.4f\t%.5f\t%10.6f\t%10.6f\t%10.6f\t%10.6f\t%10.4f\t%.5f\t%10.6f\t%10.6f\t%2d /%2d\t%s\t%14.7e\t%4d\t%12.5f\t%10.6f  \t%10.6f\t%9.3f\t%4d\t%9.3f\t%10.6f  \t%10.6f\t%12.5f\n", system.prim.t[j], system.a[j], system.e[j], system.prim.m[j], system.sec.m[j], system.prim.r[j], system.sec.r[j], system.a[0], system.e[0], system.prim.m[0], system.sec.m[0], system.prim.stage[j], system.sec.stage[j],phasehistory, system.tgw, system.prim.SN.type, system.prim.SN.w, system.prim.SN.theta, system.prim.SN.phi, system.prim.SN.vsys, system.sec.SN.type, system.sec.SN.w, system.sec.SN.theta, system.sec.SN.phi, system.sec.SN.vsys);*/
      if (j==1) maxa = system.a[0];
      if (maxa<system.a[j]) maxa = system.a[j];
/*      if ((distributionout!=NULL)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(system.prim.stage[j]==3)&&(system.sec.stage[j]==3)&&((system.prim.stage[j-1]<3)||(system.sec.stage[j-1]<3))){	//write specified system: close double WD, where one WD close to maximum mass -> inner binary of triple going to SN 1a to distribution output &&((system.prim.m[j]>1.35)||(system.sec.m[j]>1.35))&&(maxa<a_max)
        fprintf(distributionout,"%12.6e\t%9.3f\t%.5f\t%10.6f\t%10.6f\t%10.6f\t%10.6f\t%10.4f\t%.5f\t%10.6f\t%10.6f\t%2d /%2d\t%s\t%14.7e\t%4d\t%12.5f\t%10.6f  \t%10.6f\t%9.3f\t%4d\t%9.3f\t%10.6f  \t%10.6f\t%12.5f\t%9.3f\n", system.prim.t[j], system.a[j], system.e[j], system.prim.m[j], system.sec.m[j], system.prim.r[j], system.sec.r[j], system.a[0], system.e[0], system.prim.m[0], system.sec.m[0], system.prim.stage[j], system.sec.stage[j],phasehistory, system.tgw, system.prim.SN.type, system.prim.SN.w, system.prim.SN.theta, system.prim.SN.phi, system.prim.SN.vsys, system.sec.SN.type, system.sec.SN.w, system.sec.SN.theta, system.sec.SN.phi, system.sec.SN.vsys, maxa);*/
      if ((distributionout!=NULL)&&(((system.phase[j]==15)&&(system.sec.r[j]>50.0)&&(system.sec.r[j]<=4.0*system.rs[j]))||((system.phase[j]==25)&&(system.prim.r[j]>50.0)&&(system.prim.r[j]<=4.0*system.rp[j])))){	//write specified system: SN, where companion is close to fill its Roche lobe and a giant
        fprintf(distributionout,"%12.6e\t%9.3f\t%.5f\t%10.6f\t%10.6f\t%10.5f\t%10.5f\t%10.4f\t%.5f\t%10.6f\t%10.6f\t%2d /%2d\t%s\t%14.7e\t%4d\t%12.5f\t%10.6f  \t%10.6f\t%9.3f\t%4d\t%9.3f\t%10.6f  \t%10.6f\t%12.5f\t%9.3f\n", system.prim.t[j], system.a[j], system.e[j], system.prim.m[j], system.sec.m[j], system.prim.r[j], system.sec.r[j], system.a[0], system.e[0], system.prim.m[0], system.sec.m[0], system.prim.stage[j], system.sec.stage[j],phasehistory, system.tgw, system.prim.SN.type, system.prim.SN.w, system.prim.SN.theta, system.prim.SN.phi, system.prim.SN.vsys, system.sec.SN.type, system.sec.SN.w, system.sec.SN.theta, system.sec.SN.phi, system.sec.SN.vsys, maxa);
...
//        if (system.a[j]<1.0) screen=true;	//enable screen output
      ...
      if ((system.rhostar[0]==0.0)&&(system.rhostar[j]==0.0)) system.rhostar[j] = ignorev;	//field star -> no output
      ...
      if (system.M[j-1]+accuracy<system.M[j]){	//... whether the primary gained mass
        cerr << "#Error: mass increase of the system at " << j << ": dm=" << system.M[j]-system.M[j-1] << "Msun" << endl;
        screen = true;	//enable screen output
        debug = true;	//enable debugging
//        i=number;
      }
      if (system.phase[j-1]==0){	//ckeck in wind mass loss phase ...
        if ((system.prim.m[j-1]+accuracy<system.prim.m[j])&&(system.prim.stage[j-1]!=3)){	//... whether the primary gained mass
          cerr << "#Error: wind mass increase of primary: dm=" << system.prim.m[j]-system.prim.m[j-1] << "Msun" << endl;
          debug = true;	//enable debugging
        }
        if ((system.sec.m[j-1]+accuracy<system.sec.m[j])&&(system.sec.stage[j-1]!=3)){	//... whether the secondary gained mass
          cerr << "#Error: wind mass increase of secondary: dm=" << system.sec.m[j]-system.sec.m[j-1] << "Msun" << endl;
          debug = true;	//enable debugging
        }
      }
      //write to commonoutputformat # open/close file in statistics
      if ((output=='C')&&(commonoutputformat!=NULL)){
        //"ID,UID,SID,time,event,semiMajor,eccentricity,type1,mass1,radius1,Teff1,massHeCore1,type2,mass2,radius2,Teff2,massHeCore2,envBindEn,massCOCore1,massCOCore2,radiusRL1,radiusRL2,period,luminosity1,luminosity2"
        if (j==1){
          cofID++;
          fprintf(commonoutputformat,"%ld,%ld,,%.5e,%d,%.5e,%.5e,%d,%.5e,%.5e,%.5e,%.5e,%d,%.5e,%.5e,%.5e,%.5e, %.5e,%.5e,%.5e,%.5e,%.5e,%.5e,%.5e,%.5e\n", cofID, i, ignorevToNan(system.prim.t[0])*1.0e-6, cofEvent(system,0), ignorevToNan(system.a[0]), ignorevToNan(system.e[0]), cofType(system.prim,0), ignorevToNan(system.prim.m[0]), ignorevToNan(system.prim.r[0]), ignorevToNan(pow10(system.prim.lteff[0])), ignorevToNan(system.prim.cm[0]), cofType(system.sec,0), ignorevToNan(system.sec.m[0]), ignorevToNan(system.sec.r[0]), ignorevToNan(pow10(system.sec.lteff[0])), ignorevToNan(system.sec.cm[0]), ignorevToNan(getEnvelopeBindingEnergy(system,0)), ignorevToNan(system.prim.cc[0]), ignorevToNan(system.sec.cc[0]), ignorevToNan(system.rp[0]), ignorevToNan(system.rs[0]), ignorevToNan(system.P[0])/day, ignorevToNan(pow10(system.prim.llum[0])), ignorevToNan(pow10(system.sec.llum[0])));
          cofLINES++;
          cofcont = cofEvent(system,0);
        }
        if ((cofcont!=8)&&(cofcont!=81)&&(cofcont!=82)&&(cofcont!=83)&&(cofcont!=84)&&(cofcont!=85)&&(cofcont!=89)){
          fprintf(commonoutputformat,"%ld,%ld,,%.5e,%d,%.5e,%.5e,%d,%.5e,%.5e,%.5e,%.5e,%d,%.5e,%.5e,%.5e,%.5e, %.5e,%.5e,%.5e,%.5e,%.5e,%.5e,%.5e,%.5e\n", cofID, i, ignorevToNan(system.prim.t[j])*1.0e-6, cofEvent(system,j), ignorevToNan(system.a[j]), ignorevToNan(system.e[j]), cofType(system.prim,j), ignorevToNan(system.prim.m[j]), ignorevToNan(system.prim.r[j]), ignorevToNan(pow10(system.prim.lteff[j])), ignorevToNan(system.prim.cm[j]), cofType(system.sec,j), ignorevToNan(system.sec.m[j]), ignorevToNan(system.sec.r[j]), ignorevToNan(pow10(system.sec.lteff[j])), ignorevToNan(system.sec.cm[j]), ignorevToNan(getEnvelopeBindingEnergy(system,j)), ignorevToNan(system.prim.cc[j]), ignorevToNan(system.sec.cc[j]), ignorevToNan(system.rp[j]), ignorevToNan(system.rs[j]), ignorevToNan(system.P[j])/day, ignorevToNan(pow10(system.prim.llum[j])), ignorevToNan(pow10(system.sec.llum[j])));
          cofLINES++;
          cofcont = cofEvent(system,j);
        }
      }
    }
#pragma omp critical
{
    if ((output=='M')&&(((screen)&&((debug)||(wanted)||((system.phase[n]>=90)&&(system.phase[n]<=99))||(i>100)||(i==1))))){	//write out the evolution of the system	//||((system.prim.stage[n]!=3)&&(system.sec.stage[n]!=3)&&(system.phase[n]>=90)&&(system.phase[n]<=99)&&(i<100000))
      if (i==1) cout << endl << "seeds={" << seeds[0] << "," << seeds[1] << "," << seeds[2] << "," << seeds[3] << "," << seeds[4] << "," << seeds[5] << "," << seeds[6] << "," << seeds[7] << "}";
      for (j=0; j<system.n; j++){	//go through the evolution
        if (j==0){		//initial output
          if (!single) cout << endl << "discard={" << discard_mp << "," << discard_q << "," << discard_a << "," << discard_kick << "," << discard_phase << "," << discard_eccentricity << "," << discard_galactic << "," << discard_rotation << "}";
          cout << endl << "Initial ";
        }
        if ((j==n)) cout << "Final ";		//final output	//||(system.phase[j]==4)
        ...
        if (system.phase[j]>=0){	//write out binary parameters
          cout << " sys :";		//write out system parameters
          if (system.M[j]!=ignorev) cout << " M=" << system.M[j] << "Msun";	//write out total mass
          if ((system.qp[j]!=ignorev)||(system.qs[j]!=ignorev)) cout << " q=1/" << system.qp[j] << "=" << system.qs[j];	//write out mass ratio
          if (system.a[j]!=ignorev) cout << " a=" << system.a[j] << "Rsun=" << system.a[j]/au << "AU";	//write out semi-major-axis
          if (system.P[j]!=ignorev) cout << " P=" << system.P[j] << "yr=" << system.P[j]/day << "days";	//write out orbital period
          if ((system.rp[j]!=ignorev)||(system.rs[j]!=ignorev)) cout << " roche{prim/sec}={" << system.rp[j] << "/" << system.rs[j] << "}Rsun";	//write out Roche-lobe radii
          if (system.e[j]!=ignorev) cout << " e=" << system.e[j];	//write out eccentricity
        }else{	//write out system being a single star
          cout << " sys : single star";
//          if (system.M[j]!=ignorev) cout << " M=" << system.M[j] << "Msun";	//write out total mass
        }
        if (galacticintegration){
          cout << endl << "       ";
          if ((system.x[j]!=ignorev)||(system.y[j]!=ignorev)||(system.z[j]!=ignorev)) cout << " position{x,y,z}={" << system.x[j] << "," << system.y[j] << "," << system.z[j] << "}pc";	//write out galactic position
          if ((system.vx[j]!=ignorev)||(system.vy[j]!=ignorev)||(system.vz[j]!=ignorev)) cout << " velocity{x,y,z}={" << system.vx[j] << "," << system.vy[j] << "," << system.vz[j] << "}km/s";	//write out galactic velocity
          if ((system.rhostar[j]!=ignorev)&&(rhodist!=0)) cout << " rhostar=" << system.rhostar[j] << "/pc^3";	//write out stellar density
        }
        cout << endl << " prim:";	//write out parameters of the primary
        if (system.prim.m[j]!=ignorev) cout << " m=" << system.prim.m[j] << "Msun";	//write out mass
        if (system.prim.stage[j]!=6){	//write out stellar parameters
          if (system.prim.t[j]!=ignorev) cout << " t=" << system.prim.t[j] << "yr";	//write out age
          if ((system.prim.r[j]!=ignorev)&&(system.prim.omega[j]>0.0)) cout << " r_eq=" << system.prim.r[j]*rroteq(system.prim.omega[j]/sqrt(2.0/3.0*G*system.prim.m[j]/cubic(system.prim.r[j]))) << "Rsun";	//write out radius
          else if (system.prim.r[j]!=ignorev) cout << " r=" << system.prim.r[j] << "Rsun";	//write out radius
          if (system.prim.cm[j]!=system.prim.m[j]) cout << " core mass(cm)=" << system.prim.cm[j] << "Msun";	//write out core mass
          if (system.prim.llum[j]!=ignorev) cout << " lg(lum/Lsun)=" << system.prim.llum[j];	//write out luminosity
          if (system.prim.lteff[j]!=ignorev) cout << " lg(T_eff/K)=" << system.prim.lteff[j];	//write out effective temperature
          if ((system.prim.lambda[j]!=ignorev)&&(alphaTH!=-1.0)) cout << " lambda=" << system.prim.lambda[j];	//write out lambda
          if (system.prim.cc[j]!=system.prim.cm[j]) cout << " carbon core mass(cc)=" << system.prim.cc[j] << "Msun";	//write out carbon core mass
          if (system.prim.cf[j]!=ignorev) cout << " cf=" << system.prim.cf[j];	//write out concentration factor
          if ((system.prim.omega[j]>0.0)||((system.prim.omega[j]!=ignorev)&&(j==0))) cout << " v_rot=" << system.prim.omega[j]*system.prim.r[j]*rroteq(system.prim.omega[j]/sqrt(2.0/3.0*G*system.prim.m[j]/cubic(system.prim.r[j])))*1.0e-3*Rsun/yr << "km/s";	//write out rotational velocity
          //if ((system.prim.omega[j]!=ignorev)&&((system.prim.omega[j]!=0.0)||(j==0))) cout << " omega=" << system.prim.omega[j] << "/yr";	//write out angular velocity
        }
        cout << endl << "      ";
        outstagename(system.prim.stage[j],true);	//write out number and name of stage
        cout << " sec :";	//write out parameters of the secondary
        if (system.sec.m[j]!=ignorev) cout << " m=" << system.sec.m[j] << "Msun";	//write out mass
        if (system.sec.stage[j]!=6){	//write out stellar parameters
          if (system.sec.t[j]!=ignorev) cout << " t=" << system.sec.t[j] << "yr";	//write out age
          if ((system.sec.r[j]!=ignorev)&&(system.sec.omega[j]>0.0)) cout << " r_eq=" << system.sec.r[j]*rroteq(system.sec.omega[j]/sqrt(2.0/3.0*G*system.sec.m[j]/cubic(system.sec.r[j]))) << "Rsun";	//write out radius
          else if (system.sec.r[j]!=ignorev) cout << " r=" << system.sec.r[j] << "Rsun";	//write out radius
          if (system.sec.cm[j]!=system.sec.m[j]) cout << " core mass(cm)=" << system.sec.cm[j] << "Msun";	//write out core mass
          if (system.sec.llum[j]!=ignorev) cout << " lg(lum/Lsun)=" << system.sec.llum[j];	//write out luminosity
          if (system.sec.lteff[j]!=ignorev) cout << " lg(T_eff/K)=" << system.sec.lteff[j];	//write out effective temperature
          if ((system.sec.lambda[j]!=ignorev)&&(alphaTH!=-1.0)) cout << " lambda=" << system.sec.lambda[j];	//write out lambda
          if (system.sec.cc[j]!=system.sec.cm[j]) cout << " carbon core mass(cc)=" << system.sec.cc[j] << "Msun";	//write out carbon core mass
          if (system.sec.cf[j]!=ignorev) cout << " cf=" << system.sec.cf[j];	//write out cf
          if ((system.sec.omega[j]>0.0)||((system.sec.omega[j]!=ignorev)&&(j==0))) cout << " v_rot=" << system.sec.omega[j]*system.sec.r[j]*rroteq(system.sec.omega[j]/sqrt(2.0/3.0*G*system.sec.m[j]/cubic(system.sec.r[j])))*1.0e-3*Rsun/yr << "km/s";	//write out rotational velocity
          //if ((system.sec.omega[j]!=ignorev)&&((system.sec.omega[j]!=0.0)||(j==0))) cout << " omega=" << system.sec.omega[j] << "/yr";	//write out angular velocity
        }
        cout << endl << "      ";
      ...
      cout << " sys : formation: " << system.formation;	//write out formation order
...
//    debug = true;
    if (debug||single){	//write out track data
      binaryout.open("binary.csv");	// open file with name "binary.csv" to write
      binaryout << "#binary system\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t|\tprimary\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t|\tsecondary\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-\t-" << endl;	//write head of binary system track-table
      binaryout << "#j\tphase\tmass(Msun)\tmass ratio(s/p)\tsemi-major axis(Rsun)\tperiod(yr)\teccentricity\tperi-astron(Rsun)\tgalactic position{x,y,z}(pc)\tgalactic velocity{x,y,z}(km/s)\tstellar density(1/pc^3)";	//write head of binary system track-table (system)
      binaryout << "\t|\tstage\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda\tcarbon core(Msun)\tconc. factor\trotational velocity(km/s)\tRochelobe(Rsun)";	//write head of binary system track-table (primary)
      binaryout << "\t|\tstage\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda\tcarbon core(Msun)\tconc. factor\trotational velocity(km/s)\tRochelobe(Rsun)" << endl;	//write head of binary system track-table (secondary)
      for(j=0;j<system.n;j++){	//write track-table of the binary system
        binaryout << j << "\t" << system.phase[j] << "\t" << system.M[j] << "\t" << system.qs[j] << "\t" << system.a[j] << "\t" << system.P[j] << "\t" << system.e[j] << "\t" << system.peri[j] << "\t{" << system.x[j] << "," << system.y[j] << "," << system.z[j] << "}\t{" << system.vx[j] << "," << system.vy[j] << "," << system.vz[j] << "}\t" << system.rhostar[j];
        binaryout << "\t|\t" << system.prim.stage[j] << "\t" << system.prim.m[j] << "\t" << system.prim.t[j] << "\t" << system.prim.r[j] << "\t" << system.prim.cm[j] << "\t" << system.prim.llum[j] << "\t" << system.prim.lteff[j] << "\t" << system.prim.lambda[j] << "\t" << system.prim.cc[j] << "\t" << system.prim.cf[j] << "\t" << system.prim.omega[j]*system.prim.r[j]*rroteq(system.prim.omega[j]/sqrt(2.0/3.0*G*system.prim.m[j]/cubic(system.prim.r[j])))*1.0e-3*Rsun/yr << "\t" << system.rp[j];
        binaryout << "\t|\t" << system.sec.stage[j] << "\t" << system.sec.m[j] << "\t" << system.sec.t[j] << "\t" << system.sec.r[j] << "\t" << system.sec.cm[j] << "\t" << system.sec.llum[j] << "\t" << system.sec.lteff[j] << "\t" << system.sec.lambda[j] << "\t" << system.sec.cc[j] << "\t" << system.sec.cf[j] << "\t" << system.sec.omega[j]*system.sec.r[j]*rroteq(system.sec.omega[j]/sqrt(2.0/3.0*G*system.sec.m[j]/cubic(system.sec.r[j])))*1.0e-3*Rsun/yr << "\t" << system.rs[j] << endl;
      }
      binaryout.close();	//close the binary.csv-file
      primaryout.open("primary.csv");	// open file with name "primary.csv" to write
//      cerr << "primary:" << endl << "j\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda\tcarbon core(Msun)\tconc. factor\tomega (1/yr)" << endl;	//write head of primary track-table
      primaryout << "#j\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda\tcarbon core(Msun)\tconc. factor\tomega (1/yr)" << endl;	//write head of primary track-table
      for(j=0;j<system.prim.track.n;j++){	//write track-table of the primary
//        cerr << j << "\t" << system.prim.track.m[j] << "\t" << system.prim.track.t[j] << "\t" << system.prim.track.r[j] << "\t" << system.prim.track.cm[j] << "\t" << system.prim.track.llum[j] << "\t" << system.prim.track.lteff[j] << "\t" << system.prim.track.lambda[j] << "\t" << system.prim.track.cc[j] << "\t" << system.prim.track.cf[j] << "\t" << system.prim.track.omega[j] << "\t" << endl;
        if (j==0) primaryout << "max:\t";
        else primaryout << j << "\t";
        primaryout << system.prim.track.m[j] << "\t" << system.prim.track.t[j] << "\t" << system.prim.track.r[j] << "\t" << system.prim.track.cm[j] << "\t" << system.prim.track.llum[j] << "\t" << system.prim.track.lteff[j] << "\t" << system.prim.track.lambda[j] << "\t" << system.prim.track.cc[j] << "\t" << system.prim.track.cf[j] << "\t" << system.prim.track.omega[j] << endl;
      }
      primaryout.close();	//close the primary.csv-file
      secondaryout.open("secondary.csv");	// open file with name "secondary.csv" to write
//      cerr << "secondary:" << endl << "j\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda\tcarbon core(Msun)\tconc. factor\tomega (1/yr)" << endl;	//write head of secondary track-table
      secondaryout << "#j\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda\tcarbon core(Msun)\tconc. factor\tomega (1/yr)" << endl;	//write head of secondary track-table
      for(j=0;j<system.sec.track.n;j++){	//write track-table of the secondary
//        cerr << j << "\t" << system.sec.track.m[j] << "\t" << system.sec.track.t[j] << "\t" << system.sec.track.r[j] << "\t" << system.sec.track.cm[j] << "\t" << system.sec.track.llum[j] << "\t" << system.sec.track.lteff[j] << "\t" << system.sec.track.lambda[j] << "\t" << system.sec.track.cc[j] << "\t" << system.sec.track.cf[j] << "\t" << system.sec.track.omega[j] << "\t" << endl;
        if (j==0) secondaryout << "max:\t";
        else secondaryout << j << "\t";
        secondaryout << system.sec.track.m[j] << "\t" << system.sec.track.t[j] << "\t" << system.sec.track.r[j] << "\t" << system.sec.track.cm[j] << "\t" << system.sec.track.llum[j] << "\t" << system.sec.track.lteff[j] << "\t" << system.sec.track.lambda[j] << "\t" << system.sec.track.cc[j] << "\t" << system.sec.track.cf[j] << "\t" << system.sec.track.omega[j] << endl;
      }
      secondaryout.close();	//close the secondary.csv-file
    }
...
#pragma omp critical
{
    ...
    discard_rotation = discard[7]+count_rotation;	//write count into discard_rotation
...
//    if (i%5000000==0) addran++;	//increase number of loops in random seed sequence
  ...
  t2 = time(NULL);		//meassure time of the program
  AfterMainLoop();
  if(initialfromfile) systemsin.close();
  writedataout(parallel, seed, Mp_max, Mp_min, Ms_max, Ms_min, a_max, a_min, galintegrator, galpotential, done);	//writes the current data to data.out
  ...
  closeoutfiles(cofID, cofLINES, metal);	//close the output files
