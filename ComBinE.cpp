// Kruckow & Tauris 2013-2018. version 1.0.1
...
/*global variables*/
long number=10000000;		// number of calculated systems
...
// The random generator and parameters
  if (argc>1){			//check for random seed as input parameter
    ...
    if (seed==3) grid = true;	//input = -3 make a grid
...
//      seed = 13579;		//set specific seed
  ...
  if (debug){			// check tables
    ...
    for(i=0;i<nstar;i++){		//go through all masses in stararray
      ...
      cerr << "[" << i << "]: n=" << stararray[i].n << " m[0]" << stararray[i].m[0] << " t[0]" << stararray[i].t[0] << " r[0]" << stararray[i].r[0] << " cm[0]" << stararray[i].cm[0] << " llum[0]" << stararray[i].llum[0] << " lteff[0]" << stararray[i].lteff[0] << " lambda[0]" << stararray[i].lambda[0] << endl;	//write out maximum values
      for(j=1;j<min(stararray[i].n-1,999);j++){
        cerr << "[" << i << "]: j=" << j << " m[j]" << stararray[i].m[j] << " t[j]" << stararray[i].t[j] << " r[j]" << stararray[i].r[j] << " cm[j]" << stararray[i].cm[j] << " llum[j]" << stararray[i].llum[j] << " lteff[j]" << stararray[i].lteff[j] << " lambda[j]" << stararray[i].lambda[j] << endl;	//write out track point
      }
//      if ((stararray[i].TAMS>2)&&(stararray[i].n-1>stararray[i].TAMS)) cerr << "[" << i << "]: n=" << stararray[i].n << " m[TAMS]" << stararray[i].m[stararray[i].TAMS] << " t[TAMS]" << stararray[i].t[stararray[i].TAMS] << " r[TAMS]" << stararray[i].r[stararray[i].TAMS] << " cm[TAMS]" << stararray[i].cm[stararray[i].TAMS] << " llum[TAMS]" << stararray[i].llum[stararray[i].TAMS] << " lteff[TAMS]" << stararray[i].lteff[stararray[i].TAMS] << " lambda[TAMS]" << stararray[i].lambda[stararray[i].TAMS] << endl;	//write out track point at TAMS
      cerr << "[" << i << "]: n=" << stararray[i].n << " m[n-1]" << stararray[i].m[stararray[i].n-1] << " t[n-1]" << stararray[i].t[stararray[i].n-1] << " r[n-1]" << stararray[i].r[stararray[i].n-1] << " cm[n-1]" << stararray[i].cm[stararray[i].n-1] << " llum[n-1]" << stararray[i].llum[stararray[i].n-1] << " lteff[n-1]" << stararray[i].lteff[stararray[i].n-1] << " lambda[n-1]" << stararray[i].lambda[stararray[i].n-1] << endl;		//write out last track point
    }
    if (stararray2!=stararray){
      ...
      for(i=0;i<nstar2;i++){		//go through all masses in stararray2
        ...
        cerr << "[" << i << "]: n=" << stararray2[i].n << " m[0]" << stararray2[i].m[0] << " t[0]" << stararray2[i].t[0] << " r[0]" << stararray2[i].r[0] << " cm[0]" << stararray2[i].cm[0] << " llum[0]" << stararray2[i].llum[0] << " lteff[0]" << stararray2[i].lteff[0] << " lambda[0]" << stararray2[i].lambda[0] << endl;	//write out maximum values
        for(j=1;j<min(stararray2[i].n-1,999);j++){
          cerr << "[" << i << "]: j=" << j << " m[j]" << stararray2[i].m[j] << " t[j]" << stararray2[i].t[j] << " r[j]" << stararray2[i].r[j] << " cm[j]" << stararray2[i].cm[j] << " llum[j]" << stararray2[i].llum[j] << " lteff[j]" << stararray2[i].lteff[j] << " lambda[j]" << stararray2[i].lambda[j] << endl;	//write out track point
        }
//        if ((stararray2[i].TAMS>2)&&(stararray2[i].n-1>stararray2[i].TAMS)) cerr << "[" << i << "]: n=" << stararray2[i].n << " m[TAMS]" << stararray2[i].m[stararray2[i].TAMS] << " t[TAMS]" << stararray2[i].t[stararray2[i].TAMS] << " r[TAMS]" << stararray2[i].r[stararray2[i].TAMS] << " cm[TAMS]" << stararray2[i].cm[stararray2[i].TAMS] << " llum[TAMS]" << stararray2[i].llum[stararray2[i].TAMS] << " lteff[TAMS]" << stararray2[i].lteff[stararray2[i].TAMS] << " lambda[TAMS]" << stararray2[i].lambda[stararray2[i].TAMS] << endl;	//write out track point at TAMS
    ...
    for(i=0;i<nhe1;i++){		//go through all masses in hestararray1
      ...
      cerr << "[" << i << "]: n=" << hestararray1[i].n << " m[0]" << hestararray1[i].m[0] << " t[0]" << hestararray1[i].t[0] << " r[0]" << hestararray1[i].r[0] << " cm[0]" << hestararray1[i].cm[0] << " llum[0]" << hestararray1[i].llum[0] << " lteff[0]" << hestararray1[i].lteff[0] << " lambda[0]" << hestararray1[i].lambda[0] << endl;	//write out maximum values
      for(j=1;j<min(hestararray1[i].n-1,999);j++){
        cerr << "[" << i << "]: j=" << j << " m[j]" << hestararray1[i].m[j] << " t[j]" << hestararray1[i].t[j] << " r[j]" << hestararray1[i].r[j] << " cm[j]" << hestararray1[i].cm[j] << " llum[j]" << hestararray1[i].llum[j] << " lteff[j]" << hestararray1[i].lteff[j] << " lambda[j]" << hestararray1[i].lambda[j] << endl;	//write out track point
      }
//      if ((hestararray1[i].TAMS>2)&&(hestararray1[i].n-1>hestararray1[i].TAMS)) cerr << "[" << i << "]: n=" << hestararray1[i].n << " m[TAMS]" << hestararray1[i].m[hestararray1[i].TAMS] << " t[TAMS]" << hestararray1[i].t[hestararray1[i].TAMS] << " r[TAMS]" << hestararray1[i].r[hestararray1[i].TAMS] << " cm[TAMS]" << hestararray1[i].cm[hestararray1[i].TAMS] << " llum[TAMS]" << hestararray1[i].llum[hestararray1[i].TAMS] << " lteff[TAMS]" << hestararray1[i].lteff[hestararray1[i].TAMS] << " lambda[TAMS]" << hestararray1[i].lambda[hestararray1[i].TAMS] << endl;	//write out track point at TAMS
      cerr << "[" << i << "]: n=" << hestararray1[i].n << " m[n-1]" << hestararray1[i].m[hestararray1[i].n-1] << " t[n-1]" << hestararray1[i].t[hestararray1[i].n-1] << " r[n-1]" << hestararray1[i].r[hestararray1[i].n-1] << " cm[n-1]" << hestararray1[i].cm[hestararray1[i].n-1] << " llum[n-1]" << hestararray1[i].llum[hestararray1[i].n-1] << " lteff[n-1]" << hestararray1[i].lteff[hestararray1[i].n-1] << " lambda[n-1]" << hestararray1[i].lambda[hestararray1[i].n-1] << endl;		//write out last track point
    }
    cerr << "nhe " << nhe << endl;
...
#pragma omp critical
{
    if (((number>99)&&(done%(number/100)==0))||((number>100000000)&&(done%1000000==0))){	//progress: output at each per cent or every million systems
...
/*create new system/change parameters*/
    if (single){		//run single system
      ...
      initsystem(system, 0.01*round(100*Mp), round(100*Ms)/round(100*Mp), 0.01*round(100*a), e, 0.0, metal, vp, vs, x, y, z, vx, vy, vz, rhostar);	//create new system with specified parameters (at ZAMS: t=0)
      seed = newseed(3,i,0);	//set new seed for kick parameters
      seed = newseed(4,i,0);	//set new seed for phase
//      srand(i); 		//set new seed
      if (Ms/pow(10,0.05)>=Ms_min) Ms /= pow(10,0.05);	//use 20 log-steps per dex in secondary mass
      else{
        if (Mp/pow(10,0.05)>=Mp_min) Mp /= pow(10,0.05);	//use 20 log-steps per dex in primary mass
        else{
          if (a/pow(10,0.05)>=a_min) a /= pow(10,0.05);	//use 20 log-steps per dex in semi-major-axis
...
//          cout << ", (" << system.sec.track.m[j] << "," << system.sec.track.t[j] << "," << system.sec.track.r[j] << "," << system.sec.track.cm[j] << "," << system.sec.track.llum[j] << "," << system.sec.track.lteff[j] << "," << system.sec.track.lambda[j] << ")";
        }
//        cout << "]" << endl;
        // circularization
...
/*      if (system.prim.RLO||system.sec.RLO||system.prim.HeRLO||system.sec.HeRLO){	//check for Roche-lobe-overflow
    ...
    if (debug){	//write out track data for debuging
      cerr << "primary:" << endl << "j\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda" << endl;	//write head of primary track-table
      for(j=0;j<system.prim.track.n;j++){	//write track-table of the primary
        cerr << j << "\t" << system.prim.track.m[j] << "\t" << system.prim.track.t[j] << "\t" << system.prim.track.r[j] << "\t" << system.prim.track.cm[j] << "\t" << system.prim.track.llum[j] << "\t" << system.prim.track.lteff[j] << "\t" << system.prim.track.lambda[j] << "\t" << endl;
      }
      cerr << "secondary:" << endl << "j\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda" << endl;	//write head of secondary track-table
      for(j=0;j<system.sec.track.n;j++){	//write track-table of the secondary
        cerr << j << "\t" << system.sec.track.m[j] << "\t" << system.sec.track.t[j] << "\t" << system.sec.track.r[j] << "\t" << system.sec.track.cm[j] << "\t" << system.sec.track.llum[j] << "\t" << system.sec.track.lteff[j] << "\t" << system.sec.track.lambda[j] << "\t" << endl;
      }
    }
