...
void updatetrack(t_star& star, int n){
  if (star.stage[n]==0) updatestarstrack(star, n, stararray1, nstar1);	//hydrogen burning star
  else if (star.stage[n]==1) updatestarstrack(star, n, stararray2, nstar2);	//core helium burning star
  else if (star.stage[n]==2) updatestarstrack(star, n, hestararray1, nhe1);	//naked helium star
...
/*    if (star.cm[n]<newtrack.cm[1]){	//no significant core
    ...
    if ((star.stage[n]==0)&&(stararray1!=stararray2)){	//Hydrogen burning star -> check for Helium burning star
      star.stage[n] = 1;
      updatestarstrack(star, n, stararray2, nstar2);
...
/*  for (jup=1;jup<=nup;jup++){
  ...
  if (debug){
    ...
    cerr << "newtrack: newtrack.n=" << newtrack.n << endl << " j jlow jup\tmass(Msun)\tage(yr)\tradius(Rsun)\tcore mass(Msun)\tlg(lum/Lsun)\tlg(T_eff/K)\tlambda" << endl;	//write head of track-table
    cerr << " " << j << "    0   0\t" << newtrack.m[j] << "\t" << newtrack.t[j] << "\t" << newtrack.r[j] << "\t" << newtrack.cm[j] << "\t" << newtrack.llum[j] << "\t" << newtrack.lteff[j] << "\t" << newtrack.lambda[j] << "\t" << endl;	//write track-table of the star
  }
  jup = 1;
  jlow = 1;
...
//      newtrack.lambda[j] = stararray0[iup].lambda[jup]-mratio*(stararray0[iup].lambda[jup]-stararray0[ilow].lambda[jlow]);	//new lambda
      ...
      if (debug) cerr << j << "   "<< jlow << "  "<< jup << "\t" << newtrack.m[j] << "\t" << newtrack.t[j] << "\t" << newtrack.r[j] << "\t" << newtrack.cm[j] << "\t" << newtrack.llum[j] << "\t" << newtrack.lteff[j] << "\t" << newtrack.lambda[j] << endl;	//write track-table of the star	// << "\t" << cllowold << "\t" << cllow << "\t" << clupold << "\t" << clup
...
//      newtrack.lambda[j] = lambda-mratio*(lambda-stararray0[ilow].lambda[jlow]);	//new lambda
      ...
      if (debug) cerr << j << "   "<< jlow << "  "<< jup-ratioup << "\t" << newtrack.m[j] << "\t" << newtrack.t[j] << "\t" << newtrack.r[j] << "\t" << newtrack.cm[j] << "\t" << newtrack.llum[j] << "\t" << newtrack.lteff[j] << "\t" << newtrack.lambda[j] << endl;	//write track-table of the star	// << "\t" << cllowold << "\t" << cllow << "\t" << clupold << "\t" << clup
      //next trackpoint
      jlow++;
      //save old values
      cllowold = cllow;
...
//      newtrack.lambda[j] = stararray0[iup].lambda[jup]-mratio*(stararray0[iup].lambda[jup]-lambda);	//new lambda
      ...
      if (debug) cerr << j << "   "<< jlow-ratiolow << "  "<< jup << "\t" << newtrack.m[j] << "\t" << newtrack.t[j] << "\t" << newtrack.r[j] << "\t" << newtrack.cm[j] << "\t" << newtrack.llum[j] << "\t" << newtrack.lteff[j] << "\t" << newtrack.lambda[j] << endl;	//write track-table of the star	// << "\t" << cllowold << "\t" << cllow << "\t" << clupold << "\t" << clup
      //next trackpoint
      jup++;
      //save old values
      clupold = clup;
...
//  star.track.lambda[star.last] = newtrack.lambda[j]-mratio*(newtrack.lambda[j]-newtrack.lambda[j-1]);
  ...
  if (debug){
    cerr << "index\tm in Msun\tt in yr\tr in Rsun\tcm in Msun\tllum\tlteff\tlambda" << endl;	//write head of primary track-table
    cerr << star.last << "\t" << star.track.m[star.last] << "\t" << star.track.t[star.last] << "\t" << star.track.r[star.last] << "\t" << star.track.cm[star.last] << "\t" << star.track.llum[star.last] << "\t" << star.track.lteff[star.last] << "\t" << star.track.lambda[star.last] << endl;	//write new track-table of the star
  }
  ...
  for (i=star.last+1;i<star.track.n;i++){
    ...
    if (debug) cerr << i << "\t" << star.track.m[i] << "\t" << star.track.t[i] << "\t" << star.track.r[i] << "\t" << star.track.cm[i] << "\t" << star.track.llum[i] << "\t" << star.track.lteff[i] << "\t" << star.track.lambda[i] << endl;	//write new track-table of the star
    j++;
    //determine maximal values
    if (star.track.m[0]<star.track.m[i]) star.track.m[0] = star.track.m[i];
