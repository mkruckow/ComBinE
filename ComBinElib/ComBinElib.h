...
extern const double ignorev;	//value to ignore a variable
extern const int version0;	//code version
extern const int version1;	//code version
extern const int version2;	//code version
//global variables
...
extern double binfrac;		// binary fraction, how many systems are initaially binaries
...
typedef struct{
  ...
  double* cc;	//carbon core mass in Msun
  double* cf;	//concentration factor
...
typedef struct{
  ...
  double r_ini;	//separation at SN in Rsun
...
typedef struct{
  ...
  double* cf;	//concentration factor
  double* omega;	//angular velocity
  int* stage;	//evolutionary stage: 0 hydrogen burning, 1 helium burning, 2 naked helium burning, 3 WD, 4 NS, 5 BH, 6 not a star, -2 become 2, -3 become 3/4/5
  t_HRD track;	//HRD track of this star
  int rmax;	//track index where the radius is maximal
  int last;	//track index where the last used position in track is
...
/*phases:
 ...
 -1 single star
 94 (gravitational) merger of two remnants
 99 end stage/both stars are remnants
 -99 end stage/single star is a remnant
 -100 no star left
 -1000 crashed
*/
extern t_HRD savetrack;	//HRD track of a star
extern int savermax;		//rmax of a star track
void initsystem(t_system& system, double mp, double q, double a, double e, double t, double metal, double vp, double vs, double x, double y, double z, double vx, double vy, double vz, double rhostar);	//create new system with specified parameters
void initstar(t_star& star, double mass, double time, double metal, double vrot);	//create new star with specified parameters
void initnonstar(t_star& star, double mass, double time, double metal, double vrot, int kind);	//create non stellar object: compact object or very low mass object
void getbinaryparameters(t_system& system, int nextphase);	//calculates the binary parameters from its components
...
int cofType(t_star& star, int j);	//getting the stellar type for cof
int cofEvent(t_system& system, int j);	//getting the event for cof
#endif
...
extern long count_rotation;	//counts rotation generator
...
double rroteq(double om);	//getting the factor of the equatorial radius of a rotating star: R_eq = R_p * rroteq = R_* * rroteq, deMink+13
double omegacrit(double M, double R, double Gamma);	//omega of critical rotation
double GammaEdd(double L, double M, int stage);	//Big Gamma Eddington
double getOmegaMassloss(t_star& star, int n);	//calculates the angular momentum after mass loss
extern int tides;	//flag for tides
#endif
...
//extern long cnotrackupdatemerger;	//count of skipped track updates for merger products
//events
...
//endstage
extern long cfinalunknown;	//count of systems with unknown endstage
extern long ccrashed;	//count of crashed systems
extern long cdestroyed;	//count of destroyed systems, no stars left
extern long cdmerged;	//count of destroyed systems after a merger
extern long cdexploded;	//count of destroyed systems after a supernova
extern long cdunknown;	//count of destroyed systems with unknown origin
extern long cfinalsingle;	//count of systems which reach endstage as single star
extern long csborn;	//count of single star systems born as single star
//extern long csRLO;	//count of single star systems after a RLO
extern long csmerged;	//count of single star systems after a merger
extern long csexploded;	//count of single star systems after a supernova
extern long csunknown;	//count of single star systems with unknown origin
extern long cfinal;	//count of systems which reach endstage as binary
...
/*output data*/
extern bool datafile;	//write outputdata
...
extern FILE* commonoutputformat;	//enable writing to a file
/*histogramms*/
