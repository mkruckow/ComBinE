...
void initsystem(t_system& system, double mp, double q, double a, double e, double t, double metal, double vp, double vs, double x, double y, double z, double vx, double vy, double vz, double rhostar){
  ...
  if (mp<accuracy){
    ...
    if (mp>1.5*NSmax){	//BH
      initnonstar(system.prim, mp, t, metal, vp, 5);
    }else if (mp>0.15*WDmax){	//NS
      initnonstar(system.prim, mp, t, metal, vp, 4);
    }else if (mp>accuracy){	//WD
      initnonstar(system.prim, 10.0*mp, t, metal, vp, 3);
    }else{	//no star nor stellar remnant
      initnonstar(system.prim, mp, t, metal, vp, 6);
    }
  }else{
    initstar(system.prim, mp, t, metal, vp);
  }
  if (q<accuracy){
    q = fmax(-q,accuracy*0.1/mp);
    if (mp*q>1.5*NSmax){	//BH
      initnonstar(system.sec, mp*q, t, metal, vs, 5);
    }else if (mp*q>0.15*WDmax){	//NS
      initnonstar(system.sec, mp*q, t, metal, vs, 4);
    }else if (mp*q>accuracy){	//WD
      initnonstar(system.sec, 10.0*mp*q, t, metal, vs, 3);
    }else{	//no star nor stellar remnant
      initnonstar(system.sec, mp*q, t, metal, vs, 6);
    }
  }else{
...
//  star.lambda[0] = star.track.lambda[j]-tratio*(star.track.lambda[j]-star.track.lambda[j-1]);
  ...
  if (vrot==ignorev) star.omega[0] = ignorev;
  else star.omega[0] = vrot/star.r[0];	//converting rotational velocity into angular velocity
...
void outphasename(int phase){
  ...
  else if (phase==16) cout << " phase=" << phase << " planetary nebula formation around primary" << endl;
  else if (phase==26) cout << " phase=" << phase << " planetary nebula formation around secondary" << endl;
...
int cofType(t_star& star, int j){
  int stage = star.stage[j];
  if (stage==0) return 121;	//"MS star"
  else if (stage==1) return 12;	//"post-MS star"
//  else if (stage==1) return 1219;	//"post-MS star"
  else if (stage==2) return 13;	//"naked He-star"
  else if (stage==3) return 2;	//"white dwarf"
  else if (stage==4) return 3;	//"neutron star"
  else if (stage==5) return 4;	//"black hole"
  else if (stage==6) return -1;	//"no star"
  else if (stage==-2) return 1;	//"stripped star"
  else if (stage==-3) return 1;	//"exploding star"
  else return -2;
}

int cofEvent(t_system& system, int j){
  int phase = system.phase[j];
  int lastphase = system.phase[j];
  int component = 0;
  if (j>0){
    lastphase = system.phase[j-1];
    if (system.prim.stage[j-1]!=system.prim.stage[j]) component+=1;
    if (system.sec.stage[j-1]!=system.sec.stage[j]) component+=2;
  }else{
    component = 3;
  }
  if (phase==0){
    if (lastphase==12) return 41;	//post "Roche-lobe-overflow from primary star to secondary"
    else if (lastphase==21) return 42;	//post "Roche-lobe-overflow from secondary star to primary"
    else return 10+component;	//"wind mass loss"
  }
  else if (phase==12) return 31;	//"Roche-lobe-overflow from primary star to secondary"
  else if (phase==21) return 32;	//"Roche-lobe-overflow from secondary star to primary"
  else if (phase==13) return 511;	//"common envelope from primary star"
  else if (phase==23) return 512;	//"common envelope from secondary star"
  else if (phase==4) return 52; 	//"merger"
  else if (phase==15) return 21;	//"supernova explosion of primary"
  else if (phase==25) return 22;	//"supernova explosion of secondary"
  else if (phase==16) return 11;	//"planetary nebula formation around primary"
  else if (phase==26) return 12;	//"planetary nebula formation around secondary"
  else if (phase==-1) return 84;	//"single star system"
  else if (phase==94) return 52;	//"(gravitational) merger of two compact objects"
  else if (phase==99) return 82;	//"both stars are remnants"
  else if (phase==-99) return 89;	//"single star is a remnant"
  else if (phase==-100) return 85;	//"no stars left"
  else return -2;	//"undefined"
}
