...
//          infile >> lambdaB;
          ...
          lambdaG = lambda_const;
          lambdaB = 2.0*lambda_const;
        }
...
//      }while (!infile.fail());
      ...
      if (((fabs(stararray0[nstar0].t[1])<accuracy)&&(stararray0[nstar0].cm[1]<=1.0e-10))||(kind==-2)){
...
/*      if (nstar0==0){
  ...
  //close find-file
  findfile.close();
/*  cout << "[0]: n=" << stararray0[0].n << " m[0]" << stararray0[0].m[0] << " t[0]" << stararray0[0].t[0] << " r[0]" << stararray0[0].r[0] << " core mass(cm)[0]" << stararray0[0].cm[0] << " log(lum/Lsun)(llum)[0]" << stararray0[0].llum[0] << " log(T_eff/K)(lteff)[0]" << stararray0[0].lteff[0] << " lambda[0]" << stararray0[0].lambda[0] << " carbon core mass(cc)[0]" << stararray0[0].cc[0] << endl;
  cout << "[0]: n=" << stararray0[1].n << " m[1]" << stararray0[0].m[1] << " t[1]" << stararray0[0].t[1] << " r[1]" << stararray0[0].r[1] << " core mass(cm)[1]" << stararray0[0].cm[1] << " log(lum/Lsun)(llum)[1]" << stararray0[0].llum[1] << " log(T_eff/K)(lteff)[1]" << stararray0[0].lteff[1] << " lambda[1]" << stararray0[0].lambda[1] << " carbon core mass(cc)[1]" << stararray0[0].cc[1] << endl;
...
void smoothtable(double* table, int n, int monotonic){	//smooth the table to avoid steps because of the precision in input data
  ...
  for (i=0;i<n-1;i++){
    ...
    if (i+j<n) diff = table[i+j]-table[i];
    else if(i>0) diff = table[i]-table[i-1];
    else diff = table[i]*0.1*accuracy*copysign(1.0,monotonic);
    for (k=1;k<j;k++){
      table[i+k] += diff*((double)k)/((double)j);
      if (debug) cerr << table[i+k] << ", ";
    }
    i += j-1;
  }
