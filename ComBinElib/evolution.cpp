...
//  double Medd=0.0;	//Eddington limited mass for wind accretion
...
//      system.sec.lambda[n] = system.sec.track.lambda[js]-ratios*(system.sec.track.lambda[js]-system.sec.track.lambda[js-1]);	//interpolate lambda
      EbindperG = system.sec.track.m[js]*(system.sec.track.m[js]-system.sec.track.cm[js])/(system.sec.track.lambda[js]*system.sec.track.r[js])
                  -ratios*(system.sec.track.m[js]*(system.sec.track.m[js]-system.sec.track.cm[js])/(system.sec.track.lambda[js]*system.sec.track.r[js])
      ...
      //wind accretion onto WD
      //debug = true;
      /*if (system.prim.stage[n-1]==3){
        system.prim.m[n] = system.prim.m[n-1];
        if (debug) cerr << endl << "wind acc.: " << system.prim.m[n] << "Msun -> ";
        for (j=1;j<js;j++){
          if (system.sec.track.t[j]>system.sec.t[n-1]){
            if (system.sec.stage[n-1]<2) Medd = system.prim.r[n-1]*4.0*M_PI*MH*c/(sigmaT)*(system.sec.track.t[j]-fmax(system.sec.track.t[j-1], system.sec.t[n-1]));	//accrete H
            else Medd = system.prim.r[n-1]*4.0*M_PI*4.002603/1.007825*MH*c/(2.0*sigmaT)*(system.sec.track.t[j]-fmax(system.sec.track.t[j-1], system.sec.t[n-1]));	//accrete He
            system.prim.m[n] += fmin(0.1*square(system.sec.track.r[j-1]/rocherad(system.a[n-1]*(system.M[n-1])/(fmin(system.sec.track.m[j-1], system.sec.m[n-1])+system.prim.m[n]), fmin(system.sec.track.m[j-1], system.sec.m[n-1])/system.prim.m[n]))*(fmin(system.sec.track.m[j-1], system.sec.m[n-1])-system.sec.track.m[j]), Medd);
            if (debug) cerr << "+" << Medd << "<=" << system.prim.m[n] << "Msun -> ";
          }
        }
        if (system.sec.stage[n-1]<2) Medd = system.prim.r[n-1]*4.0*M_PI*MH*c/(sigmaT)*(t-fmax(system.sec.track.t[js-1], system.sec.t[n-1]));	//accrete H
        else Medd = system.prim.r[n-1]*4.0*M_PI*4.002603/1.007825*MH*c/(2.0*sigmaT)*(t-fmax(system.sec.track.t[js-1], system.sec.t[n-1]));	//accrete He
        system.prim.m[n] += fmin(0.1*square(system.sec.track.r[js-1]/rocherad(system.a[n-1]*(system.M[n-1])/(fmin(system.sec.track.m[js-1], system.sec.m[n-1])+system.prim.m[n]), fmin(system.sec.track.m[js-1], system.sec.m[n-1])/system.prim.m[n]))*(fmin(system.sec.track.m[js-1], system.sec.m[n-1])-system.sec.m[n]), Medd);
        if (debug) cerr << system.prim.m[n] << "Msun" << endl;
        if (debug) cerr << system.prim.m[n-1] << "Msun -> " << system.prim.m[n] << "Msun" << endl;
      }
      if (system.sec.stage[n-1]==3){
        system.sec.m[n] = system.sec.m[n-1];
        if (debug) cerr << endl << "wind acc.: " << system.sec.m[n] << "Msun -> ";
        for (j=1;j<jp;j++){
          if (system.prim.track.t[j]>system.prim.t[n-1]){
            if (system.prim.stage[n-1]<2) Medd = system.sec.r[n-1]*4.0*M_PI*MH*c/(sigmaT)*(system.prim.track.t[j]-fmax(system.prim.track.t[j-1], system.prim.t[n-1]));	//accrete H
            else Medd = system.sec.r[n-1]*4.0*M_PI*4.002603/1.007825*MH*c/(2.0*sigmaT)*(system.prim.track.t[j]-fmax(system.prim.track.t[j-1], system.prim.t[n-1]));	//accrete He
            system.sec.m[n] += fmin(0.1*square(system.prim.track.r[j-1]/rocherad(system.a[n-1]*(system.M[n-1])/(fmin(system.prim.track.m[j-1], system.prim.m[n-1])+system.sec.m[n]), fmin(system.prim.track.m[j-1], system.prim.m[n-1])/system.sec.m[n]))*(fmin(system.prim.track.m[j-1], system.prim.m[n-1])-system.prim.track.m[j]), Medd);
            if (debug) cerr << "+" << Medd << "<=" << system.sec.m[n] << "Msun -> ";
          }
        }
        if (system.prim.stage[n-1]<2) Medd = system.sec.r[n-1]*4.0*M_PI*MH*c/(sigmaT)*(t-fmax(system.prim.track.t[jp-1], system.prim.t[n-1]));	//accrete H
        else Medd = system.sec.r[n-1]*4.0*M_PI*4.002603/1.007825*MH*c/(2.0*sigmaT)*(t-fmax(system.prim.track.t[jp-1], system.prim.t[n-1]));	//accrete He
        system.sec.m[n] += fmin(0.1*square(system.prim.track.r[jp-1]/rocherad(system.a[n-1]*(system.M[n-1])/(fmin(system.prim.track.m[jp-1], system.prim.m[n-1])+system.sec.m[n]), fmin(system.prim.track.m[jp-1], system.prim.m[n-1])/system.sec.m[n]))*(fmin(system.prim.track.m[jp-1], system.prim.m[n-1])-system.prim.m[n]), Medd);
        if (debug) cerr << system.sec.m[n] << "Msun" << endl;
        if (debug) cerr << system.prim.m[n-1] << "Msun -> " << system.prim.m[n] << "Msun" << endl;
      }*/
      //debug = false;
      ...
      if ((((nextphase==12)&&(fabs(1.0-system.prim.r[n]/system.rp[n])>0.01))||((nextphase==21)&&(fabs(1.0-system.sec.r[n]/system.rs[n])>0.01)))&&(t>0.0)){	//check if the Roche-lobe filling star does so and if the time is in the future
...
//    Medd = accretor[0].r[n]*Rsun*1.5E-12;	//Eddington-limit: Medd = R_acc/(10km) * 1.5E-8 M_sun/yr
    Medd = accretor[0].r[n]*4.0*M_PI*MH*c/(sigmaT);	//Eddington-limit: Medd = 4*pi*(mass of accreted object)*c/(count*Thomson cross section of radiative influenced particle per accreted object) here: accreted material=hydrogen, radiative influenced particle=electron	//Eddington-limit: Medd = (accretor size)*4*pi*(mass of accreted material)*c/(count*Thomson cross section of radiative influenced particle per accreted object) here: accreted material=hydrogen, radiative influenced particle=electron
...
//    Medd = accretor[0].r[n]*Rsun*3.0E-12;	//Eddington-limit: Medd = R_acc/10km * 2 * 1.5E-8 M_sun/yr
    ...
    if (accretor[0].stage[n]>2){	//accretor is compact object
      ...
      Medd = 4.0*M_PI*G*accretor[0].m[n]*4.002603/1.007825*MH*c/(2.0*sigmaT*(E_acc+E_nuc));	//M_edd=L_edd/(epsilon_acc+epsilon_nuc) and L=4*pi*G*M_accretor*m_paricle*c/(count_{electrons per particle}*sigma_T), here count_{electrons per particle}=2
    }
...
/*    if (donor[0].stage[n]<2) donor[0].RLO = true;	//lose hydrogen
  ...
  //todo: Chen, Woods, Yungelson et al. 2019
...
//  int cstagechangestar=0, cstagechangecompanion=0;	//count of stage changes
  ...
  double EbindperG=0.0;	//envelope binding energy for interpolation
  double dim=2.5;	//dimension in the angular momentum formalism
  double eta=2.0;	//efficiency in the angular momentum formalism
  double mc=0.0;	//core mass in the angular momentum formalism
  double me=0.0;	//envelope mass in the angular momentum formalism
  double m2=0.0;	//companion mass in the angular momentum formalism
  double fstar=0.0;	//mass influence function of the star's core
  double fcompanion=0.0;	//mass influence function of the companion
  ...
  if (star[0].cm[n]<accuracy) return 4;	//no core to be left -> merge
...
//  }
  ...
  if ((lambda<=0.0)){	//extrapolated lambda which lead to zero or negative semi-major-axis	//||(alphaCE==0.0)
    system.a[n] = 1.0e-99;
  }else if (alphaCE>0.0){	//determine semi-major-axis change via alpha-lambda formalism
  ...
  }else{	//use angular momentum formalism
    mc = star[0].cm[n];
    me = star[0].m[n]-star[0].cm[n];
    m2 = companion[0].m[n];
    if(lambda_const<0) dim = -lambda_const;
    fstar = pow(rocherad(1.0, mc/m2),dim);
    fcompanion = pow(rocherad(1.0, m2/mc),dim);
    if (alphaCE==0.0){
      eta = 0.551282578627741*mc/m2+2.10261092590999*mc/me+5.14995565850226*m2/me-0.157701692068785*me/m2;	//fitting function
    }else{
      eta = -alphaCE;
    }
    system.a[n] *= (m2+mc)*square(1.0+me/mc)/(m2+mc+me)*exp(-2.0*eta*(me/(m2+mc))*(fstar*m2/mc+fcompanion*mc/m2)/(fstar+fcompanion));
    if (screen && (output=='M')) cout << " f_{core}=" << fstar << " f_{comp}=" << fcompanion << " eta=" << eta << " mc=" << mc << " me=" << me << " m2=" << m2;
  }
...
//    cstagechangecompanion += 1;
  ...
  if (companion[0].stage[n]!=-3){
    ...
    for (j=max(1,companion[0].last);j<companion[0].track.n-1;j++){	//find time index of companion[0].t[n]
...
//    cstagechangestar += 1;
    ...
    if ((rocherad(system.a[n],star[0].m[n]/companion[0].m[n])<star[0].r[n])||(star[0].m[n]<0.01)){	// merging (via instant case A RLO)
...
//      if (debug) cerr << " cstagechangestar=" << cstagechangestar << " cstagechangecompanion=" << cstagechangecompanion << endl;
      if ((screen)&&(output=='M')) cout << " donor RLO: a=" << system.a[n] << "Rsun R_L=" << rocherad(system.a[n],star[0].m[n]/companion[0].m[n]) << "Rsun R=" << star[0].r[n] << "Rsun";
...
//      if (debug) cerr << " cstagechangestar=" << cstagechangestar << " cstagechangecompanion=" << cstagechangecompanion << endl;
      if ((screen)&&(output=='M')) cout << " companion RLO: a=" << system.a[n] << "Rsun R_L=" << rocherad(system.a[n],companion[0].m[n]/star[0].m[n]) << "Rsun R=" << companion[0].r[n] << "Rsun";
      undophase(star[0], n, false);		//reset phase for star
      undophase(companion[0], n, false);	//reset phase for companion
//      if (cstagechangestar>0) setlasttrack(star[0], n);			//reset last track to compensate nextstage
//      if (cstagechangecompanion>0) setlasttrack(companion[0], n);	//reset last track to compensate nextstage
...
int merge(t_system& system){
  ...
  if (screen && (output=='M')) cout << "mass_merger=" << merger[0].m[n] << "Msun core mass_merger=" << merger[0].cm[n] << "Msun carbon-core mass_merger=" << merger[0].cc[n] << "Msun time=" << tcoal << "yr";
  if (merger[0].stage[n]<=2){	//merger product will evolve further
    ...
    if (merger[0].t[n]+tcoal>merger[0].track.t[merger[0].track.n-1]){	//maximal time for merger product
      merger[0].t[n] = merger[0].track.t[merger[0].track.n-2];
      tcoal = merger[0].t[n]-merger[0].t[n-1];
//      if (merger[0].stage[n]>0) merger[0].stage[n] = -3;	//merger explodes
    }else{
    ...
    if (debug) cerr << "stage_merger=" << merger[0].stage[n] << endl;
    if (merger[0].stage[n]<0) merger[0].track.cc[merger[0].last] = merger[0].cc[n];	// star at end of grid: don't update carbon core mass -> overwrite track
    else merger[0].cc[n] = merger[0].track.cc[merger[0].last];
...
/*    if (star[0].m[n]==star[0].cm[n]){
  ...
  if (debug||screen){
//    cout << "BH_limit=" << BH_limit << "Msun NS_limit=" << NS_limit << "Msun NS_CC_limit=" << NS_CC_limit << "Msun CO_limit=" << CO_limit << "Msun ";
    cout << "star[0].inimass=" << star[0].inimass << "Msun McoreCO=" << McoreCO << "Msun MlayerHe=" << MlayerHe << "Msun" << endl;
  }
...
//    star[0].m[n] = fmin(fmax(McoreCO,0.6),star[0].SN.remnantmass);	//get remnant mass in Msun of white dwarf being a CO-WD with m>=0.6
    if (McoreCO>MlayerHe) star[0].m[n] = McoreCO+1.0e-2*MlayerHe;	//get remnant mass in Msun of white dwarf being a CO-WD having 1% He on top
    else if (star[0].SN.remnantmass>0.08) star[0].m[n] = McoreCO+MlayerHe+1.0e-4*(star[0].SN.remnantmass-star[0].SN.Hecoremass);	//get remnant mass in Msun of white dwarf being a He-WD having 0.01% H on top
    else star[0].m[n] = star[0].SN.remnantmass;	//get remnant mass in Msun of white dwarf being a H-WD
    if (debug||screen) cout << "star[0].SN.remnantmass=" << star[0].SN.remnantmass << "Msun star[0].SN.Hecoremass=" << star[0].SN.Hecoremass << "Msun star[0].m[n]=" << star[0].m[n] << "Msun cf. " << fmin(fmax(McoreCO,0.6),star[0].SN.remnantmass) << "Msun" << endl;
    if (star[0].m[n]>WDmax){
      if (!debug){
        cout << "BH_limit=" << BH_limit << "Msun NS_limit=" << NS_limit << "Msun NS_CC_limit=" << NS_CC_limit << "Msun CO_limit=" << CO_limit << "Msun ";
        cout << "star[0].inimass=" << star[0].inimass << "Msun McoreCO=" << McoreCO << "Msun MlayerHe=" << MlayerHe << "Msun" << endl;
...
//  if (debug) cerr << endl << "wx=" << wx/kms << "km/s wy=" << wy/kms << "km/s wz=" << wz/kms << "km/s";
  ...
  if(system.a[n]==ignorev){	//not a binary (single star)
    v = accuracy;
    r_ini = system.a[n];
  }else if(system.e[n]<1.0E-5){	//circular orbit
...
//    if (companion[0].stage[n]==-3) companion[0].stage[n] = companion[0].stage[n-1];
    if (companion[0].stage[n]==-3){	//companion will explode too
      //determine values for the system
      getbinaryparameters(system, -99);	//get new binary parameters
