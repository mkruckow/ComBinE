...
//     Note here, q=M_donor/M_accretor:
       return a*0.49*pow(q,2./3.)/(0.6*pow(q,2./3.)+log(1.+pow(q,1./3.)));
}
...
//RLFT: Roche-lobe-filling time
  ...
  double acirc=system.a[n]*(1.0-pow(system.e[n],2));	//circuarised semi-major-axis
...
double WDradius(double M){	//calculate radius of a White Dwarf
  return 0.013*pow(M,-1.0/3.0);	//non-relativistic WD
}
