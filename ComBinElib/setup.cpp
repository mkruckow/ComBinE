...
mt19937_64 generator_mp,generator_q,generator_a,generator_kick,generator_phase,generator_eccentricity,generator_galactic,generator_rotation;	//random number generator
mt19937_64 save_mp,save_q,save_a,save_kick,save_phase,save_eccentricity,save_galactic,save_rotation;	//saves of random number generator
...
long count_rotation=0;	//counts rotation generator
...
/*    save_galactic.seed(s);
  ...
  }else if (i==7){	//rotation
    if (s==0) s = generator_rotation();	//get random value
    generator_rotation.seed(s);	//set seed
    count_rotation = 0;
    //create image of the generatorstatus
/*    save_rotation.seed(s);
    save_rotaiton.discard(d);*/
    save_rotation = generator_rotation;
    //make one step
    generator_rotation.discard(d);	//discard d+1 random numbers
  }else{
...
#pragma omp critical
{
    r = generator_rotation();
    if (save_rotation==generator_rotation){
//      if (newseed(i,r+1,0)!=(long)r) r=generator_rotation();
      newseed(i,r+1,0);
      r = generator_rotation();
    }
    m = generator_rotation.max();
    count_rotation++;
}	//end critical
  }else{
...
// Using a Monte Carlo drawing to find initial Mp, q and a
  ...
  const double P_min_Sana = pow(10.0,0.15);	//lower limit of Sana's period distribution: 10^0.15 days (private communication)
  static double kP;
  double e_max;
...
//  const double etaP=2.5;
  double Mp, q, a, e, t, vp, vs, x, y, z, rho, phi, vx, vy, vz, ax, ay, az, rhostar;
  double Ms;
...
//Picking random masses for the primary star between Mp_min and Mp_max in Msun
  ...
  if (IMF==1){
    //use Salpeter-like IMF (Scalo 1986; Kroupa, Tout & Gilmore 1993)
...
//Picking the mass ratio between q_min(Ms_min,Mp) and q_max(Ms_max,Mp) where q=Ms/Mp
  ...
  if (ranv(1)>binfrac){	//single star or binary system
    q = 0.1*accuracy;
  }else if (qdist==1){
...
//Picking random separations between a_min and a_max in Rsun
  ...
  if (q<accuracy){	//single star
    ...
    P_min = 2.0*M_PI*sqrt(cubic(a_min)/(G*Mp*(1.0+q)))/day;	//get minimum Period in days
    P_max = 2.0*M_PI*sqrt(cubic(a_max)/(G*Mp*(1.0+q)))/day;	//get maximum Period in days
    kP = log(square(log10(P_max/P_min))/deltaP+1.0);	//get normalisation in original formular kp=2.0/eta
    P = pow(10.0,sqrt(deltaP*(exp(kP*ranv(2))-1.0)))*P_min;	//generate random a from P-distribution in days
    a = cbrt(square(P*day/(2.0*M_PI))*(G*Mp*(1.0+q)));	//convert back to semi-major axis in Rsun
  }else if (adist==3){
    //period distribution from Sana et al.(2012) f(P) ~ P^-0.55
    P_min = 2.0*M_PI*sqrt(cubic(a_min)/(G*Mp*(1.0+q)))/day;	//get minimum Period in days
    if (P_min<P_min_Sana){	//check lower limit = 10^0.15 days (private communication)
      if (screen) cerr << "#Warning: readjust minimal period to " << P_min_Sana << "days" << endl;
      P_min = P_min_Sana;
    }
    P_max = 2.0*M_PI*sqrt(cubic(a_max)/(G*Mp*(1.0+q)))/day;	//get maximum Period in days
    P_min = log10(P_min);	//convert to log10 values
    P_max = log10(P_max);	//convert to log10 values
    ...
    P = pow(10.0,P);		//convert back to periods
    a = cbrt(square(P*day/(2.0*M_PI))*(G*Mp*(1.0+q)));	//convert back to semi-major axis in Rsun
  }else if (adist<-9){
    a = a_min-1.0;
...
//Picking random eccentricities between 0 and 1
  ...
  if (edist==0){
    ...
    e = sqrt(1.0-square(sqrt(1.0-e_max*e_max)+(1.0-sqrt(1.0-e_max*e_max))*ranv(5)));
  }else if (edist==4){
...
//Picking initial angular velocity
  ...
  if (rotdist==-1){
    //synchronised
    vp = 1.0/(2.0*M_PI*sqrt(cubic(a)/(G*Mp*(1.0+q))));
    vs = vp;
  }else if (rotdist==0){
    vp = 0.0;	//non rotating primary
    vs = 0.0;	//non rotating secondary
  }else if (rotdist>0){	//flat in v_rot
    vp = ranv(7)*rotdist*1.0e+3/Rsun*yr;	//rotation of the primary
    vs = ranv(7)*rotdist*1.0e+3/Rsun*yr;	//rotation of the secondary
  }else if (rotdist<-9){
    UserInitial_vrot(vp, vs, Mp, Ms, a, e, t, metal);
  }else{
    cerr << "#Error: No initial stellar spin distribution spezified! vp and vs set to 0.0" << endl;
    vp = 0.0;	//non rotating primary
    vs = 0.0;	//non rotating secondary
  }
  ...
  if (Rdist==-1){
    ...
    UserInitial_R(x, y, z, Mp, Ms, a, e, t, metal, vp, vs);
  }else{
...
//    cout << endl << "a={" << ax << ", " << ay << "," << az << "}\tr={" << x << ", " << y << "," << z << "}\tv={" << vx << ", " << vy << "," << vz << "}" << endl;
    ...
    UserInitial_V(vx, vy, vz, Mp, Ms, a, e, t, metal, vp, vs, x, y, z);
  }else{
  ...
  if (rhodist==0){
    ...
    rhostar = UserInitial_rho(Mp, Ms, a, e, t, metal, vp, vs, x, y, z, vx, vy, vz);
  }else{
    cerr << "#Error: No initial stellar density distribution in a galaxy spezified! star is set in the field" << endl;
    rhostar = 0.0;	//field star
  }

  if (screen) cout << "System parameters: M_prim(Mp)=" << Mp << "Msun mass ratio(q)=" << q << " semi-major-axis(a)=" << a << "Rsun eccentricity(e)=" << e << " age(t)=" << t << "yr metallicity=" << metal << " v_rot_prim(vp)=" << vp << "/yr v_rot_sec(vs)=" << vs << "/yr galactic position(x,y,z)=(" << x << ", " << y << ", " << z << ")pc galactic velocity(vx,vy,vz)=(" << vx << ", " << vy << ", " << vz << ")km/s stellar density(rhostar)=" << rhostar << "/pc^3" << endl;
  initsystem(system, Mp, q, a, e, t, metal, vp, vs, x, y, z, vx, vy, vz, rhostar);	//create new system with specified parameters
}
...
void getMWposition(double& rho, double& phi, double& z){	//get position disk of MW_potential by Allen, C., Santillan, A., 1991, RevMexAA, 22, 255
  ...
  const double factor = cubic(a2+b2)*cubic(b2)/(a2+3.0*b2);
  const double g = 200.0;
  const double g3 = cbrt(g);
  double d,zb;
  double ran, frho;
  do{
    ran = ranv(6);
    if (ran<2.0/3.0){
      ...
      rho = g3/sqrt(3.0-3.0*ran);
      frho = g/cubic(rho);
    }
//    rho = 1000.0*ranv(6);	//get rho uniform in [0,1000[
    z = 44.0*ranv(6)-22.0;		//get z uniform in [-22,22[
    zb = sqrt(z*z+b2*b2);
    d = (a2*rho*rho+(a2+3.0*zb)*(a2+zb)*(a2+zb))*pow(rho*rho+(a2+zb)*(a2+zb),-2.5)/cubic(zb)*factor;
  }while(d<frho*ranv(6));
