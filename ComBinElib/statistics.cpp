...
//long cnotrackupdatemerger=0;	//count of skipped track updates for merger products
//events
...
//endstage
long cfinalunknown=0;	//count of systems with unknown endstage
long ccrashed=0;	//count of crashed systems
long cdestroyed=0;	//count of destroyed systems, no stars left
long cdmerged=0;	//count of destroyed systems after a merger
long cdexploded=0;	//count of destroyed systems after a supernova
long cdunknown=0;	//count of destroyed systems with unknown origin
long cfinalsingle=0;	//count of systems which reach endstage as single star
long csborn=0;		//count of single star systems born as single star
//long csRLO=0;		//count of single star systems after a RLO
long csmerged=0;	//count of single star systems after a merger
long csexploded=0;	//count of single star systems after a supernova
long csunknown=0;	//count of single star systems with unknown origin
long cfinal=0;	//count of systems which reach endstage as binary
...
/*output data*/
bool datafile=true;	//write outputdata
...
FILE* commonoutputformat=NULL;	//enable writing to a file
long savepos;
...
int getformation(int f){	//returns array index of formation
  if (f==33) return 1;
  ...
  //else if (f%10==6) return 10;
  else return 0;
}

const char * getheader(int i){	//returns fromation text according to array index
  if (i==1) return "WD-WD";
...
/*    fprintf(distributionout,"#current\t         \t       \t          \t          \t          \t          \tinitial   \t       \t          \t          \tevolution     \tpost evolution\tprimary SN   \t             \t             \t    \t         \t            \t          \tsecondary SN \t             \t             \t    \t         \t            \t          \n");
  ...
  if (output=='C'){
    commonoutputformat=fopen("CommonOutputFormat.csv","w");	// open file with name "CommonOutputFormat.csv" to write
    if (commonoutputformat!=NULL){
      //Free-Format section
//      fprintf(commonoutputformat,"# %e is an unset value\n",ignorev);
      fprintf(commonoutputformat,"#constants (next lines: names, values in SI units)\n");
//      fprintf(commonoutputformat,"#constants (next lines: names, values in cgs units)\n");
      fprintf(commonoutputformat,"#Msun,Rsun,year,Lsun,Ggrav,clight,MH,sigmaT,day,AU,pc\n");
      fprintf(commonoutputformat,"#%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e\n", Msun, Rsun, yr, Lsun*Msun*Rsun*Rsun/(yr*yr*yr), G*(Rsun*Rsun*Rsun)/(Msun*yr*yr), c*Rsun/yr, MH*Msun, sigmaT*Rsun*Rsun, day*yr, au*Rsun, pc*Rsun);
//      fprintf(commonoutputformat,"#%e,%e,%e,%e,%e,%e,%e,%e,%e,%e,%e\n",Msun*1.0e+3, Rsun*1.0e+2, yr, Lsun/(cgsEnergy*yr), G*(Rsun*Rsun*Rsun*1.0e+6)/(Msun*1.0e+3*yr*yr), c*Rsun*1.0e+2/yr, MH*Msun*1.0e+3, sigmaT*Rsun*Rsun*1.0e+4, day*yr, au*Rsun*1.0e+2, pc*Rsun*1.0e+2);
      //Header
      fprintf(commonoutputformat,"cofVer,cofLevel,cofExtension,bpsName,bpsVer,contact,NSYS,NLINES,Z\n");
      savepos = ftell(commonoutputformat);
      fprintf(commonoutputformat,"1.0,L1,,ComBinE,%d.%d.%d,mkruckow@astro.uni-bonn.de,000000000000,000000000000,0.00e+00\n", version0, version1, version2);
      fprintf(commonoutputformat,"ID,UID,SID,time,event,semiMajor,eccentricity,type1,mass1,radius1,Teff1,massHeCore1,type2,mass2,radius2,Teff2,massHeCore2, envBindEn,massCOCore1,massCOCore2,radiusRL1,radiusRL2,period,luminosity1,luminosity2\n");
      fflush(commonoutputformat);	//update file
    }
  }
}
...
void writedataout(int parallel, long seed, double Mp_max, double Mp_min, double Ms_max, double Ms_min, double a_max, double a_min, int galintegrator, int galpotential, long done){ 
  if (datafile){
    ...
    dataout << "#parallel\tseed\tnumber\t(Mp_max,Mp_min)\t(Ms_max,Ms_min)\t(a_max,a_min)\talphaRLO\tbeta_const\tGamma\tdelta\talphaCE\tlambda_const\tqlimit\tMhece\tkickv\talphaTH\tgalintegrator\tgalpotential\ttides\toutput\tIMF\tbinfrac\tqdist\tadist\tedist\ttdist\tZdist\trotdist\tRdist\tVdist\trhodist" << endl;
    dataout << parallel << "\t" << seed << "\t" << number << "\t(" << Mp_max << "," << Mp_min << ")\t(" << Ms_max << "," << Ms_min << ")\t(" << a_max << "," << a_min << ")\t" << alphaRLO << "\t" << beta_const << "\t" << Gamma << "\t" << delta << "\t" << alphaCE << "\t" << lambda_const << "\t" << qlimit << "\t" << Mhece << "\t" << kickv << "\t" << alphaTH << "\t" << galintegrator << "\t" << galpotential << "\t" << tides << "\t" << output << "\t" << IMF << "\t" << binfrac << "\t" << qdist << "\t" << adist << "\t" << edist << "\t" << tdist << "\t" << Zdist << "\t" << rotdist << "\t" << Rdist << "\t" << Vdist << "\t" << rhodist << endl;
    ...
    dataout << "#number of systems\tsurvive(SB)\tdestroyed(DB)\tunknown" << endl;
    dataout << done << "\t" << cfinal << "\t" << cfinalsingle+cdestroyed+ccrashed << "\t" << cfinalunknown << endl;
    dataout << "#number of SB\tWD-WD\tWD-NS\tWD-BH\tNS-WD\tNS-NS\tNS-BH\tBH-WD\tBH-NS\tBH-BH\tunknown" << endl;
//    dataout << cfinal << "\t" << cWDWD << "\t" << cWDNS << "\t" << cWDBH << "\t" << cNSWD << "\t" << cNSNS << "\t" << cNSBH << "\t" << cBHWD << "\t" << cBHNS << "\t" << cBHBH << endl;
    ...
    dataout << "#SB t_gw<" << tmax << "yr\tWD-WD\tWD-NS\tWD-BH\tNS-WD\tNS-NS\tNS-BH\tBH-WD\tBH-NS\tBH-BH\tunknown" << endl;
//    dataout << cgw << "\t" << cgwWDWD << "\t" << cgwWDNS << "\t" << cgwWDBH << "\t" << cgwNSWD << "\t" << cgwNSNS << "\t" << cgwNSBH << "\t" << cgwBHWD << "\t" << cgwBHNS << "\t" << cgwBHBH << endl;
    ...
    dataout << "#number of DB\tsingle star(SS)\tno star(No)\tcrashed" << endl;
    dataout << cfinalsingle+cdestroyed+ccrashed << "\t" << cfinalsingle << "\t" << cdestroyed << "\t" << ccrashed << endl;
    dataout << "#number of SS\tmerged sys.\tdisrupted sys.\tborn as SS\tunknown" << endl;
    dataout << cfinalsingle << "\t" << csmerged << "\t" << csexploded << "\t" << csborn << "\t" << csunknown << endl;
    dataout << "#number of No\tmerged sys.\tdisrupted sys.\tunknown" << endl;
    dataout << cdestroyed << "\t" << cdmerged << "\t" << cdexploded << "\t" << cdunknown << endl;
    dataout.close();	//close the data.out-file
  }
}

void closeoutfiles(long cofID, long cofLINES, double metal){	//close output files 
  ...
  if (commonoutputformat!=NULL){
    fseek(commonoutputformat,savepos,SEEK_SET);
    fprintf(commonoutputformat,"1.0,L1,,ComBinE,%d.%d.%d,mkruckow@astro.uni-bonn.de,%12ld,%12ld,%.2e\n", version0, version1, version2, cofID, cofLINES+6, metal);
    fclose(commonoutputformat);
  }
...
void adddata(t_system& system, long i){
  ...
  //counters
  if ((system.phase[n]>=90)&&(system.phase[n]<=99)){	//system get to endstage as binary
    cfinal++;
#pragma omp critical
{
...
/*    if (system.prim.stage[n]==3){	//primary is a white dwarf
    ...
    system.tgw = fmax(mergetime(system.prim.m[n],system.sec.m[n],system.a[n],system.e[n]),0.5*system.P[n]);	//merger take at least half an orbit
//    if (system.tgw<1.0) screen = true;
...
//    if (plot) addtohist(tmerge,system.tgw,sub);
  ...
  }else if (system.phase[n]==-99){	//system get to endstage as single star
    cfinalsingle++;
    if (system.phase[0]==0){	//born as binary
      for (j=1; j<=n; j++){
        if (system.phase[j]<0){	//first single star phase
          if (system.phase[j-1]==4) csmerged++;	//after merger
          else if (system.phase[j-1]%10==5){	//after supernova
            if (system.phase[max(0,j-2)]==4) csmerged++;	//merger caused single star with direct supernova
            else csexploded++;
          }else if ((system.phase[j-1]%10==6)&&(system.phase[max(0,j-2)]==4)){	//after PN
            csmerged++;	//merger caused single star with direct PN
          }else if ((system.phase[j-1]%10==6)&&(system.phase[max(0,j-2)]%10==5)){	//after PN
            csexploded++;	//previous SN caused single star with direct PN
          }else{
            cerr << "#got single star: unknown" << endl;
            screen = true;
            csunknown++;
          }
          break;
    ...
    }else if (system.phase[0]==-1){	//born as single star
      csborn++;
    }else{
      cerr << "#single star: unknown birth" << endl;
      screen = true;
      csunknown++;
    }
  }else if (system.phase[n]==-100){	//system is destroyed, no stars left
    cdestroyed++;
    if (system.phase[n-1]==4) cdmerged++;	//system is destroyed by a merger ((system.phase[n-1]==4)||((system.phase[n-1]==-1)&&(system.phase[max(0,n-2)]==4)))
    else if ((system.phase[n-1]%10==5)||((system.phase[n-1]==-1)&&(system.phase[max(0,n-2)]%10==5))) cdexploded++;	//system is destroyed by a supernova
    else {
      cerr << "#destroyed: unknown" << endl;
      screen = true;
      cdunknown++;
    }
  }else if (system.phase[n]==-1000){	//crashed systems
    ccrashed++;
  }else{	//unknown endstage
    cerr << "#unknown final phase" << endl;
    screen = true;
    cfinalunknown++;
  }
  ...
  //grid output
  if (grid){
    if ((system.phase[n]>=90)&&(system.phase[n]<=99)) gridout << i << "\t" << system.prim.m[0] << "\t" << system.sec.m[0] << "\t" << system.a[0] << "\t" << system.phase[n] << "\t" << system.prim.stage[n] << "\t" << system.sec.stage[n] << "\t" << system.prim.m[n] << "\t" << system.sec.m[n] << "\t" << system.a[n] << "\t" << system.tgw << endl;	// << "\t" << system.sec.cm[n] << "\t" << system.sec.cc[n]
...
//  if (datafile && inform[5]==1 && inform[10]==1) dataout << endl << Mp[0] << " " << Ms[0] << " " << a[0] << " "
...
//if (datafile && (inform[5]==1 || inform[5]==2) && (inform[10]==1 || inform[10]==2) && (inform[6]==1))
  if (datafile && (inform[5]==1 || inform[5]==2) && (inform[10]==1 || inform[10]==2)) 	//NS/BH+NS/BH
//if (datafile && (inform[5]==2) && (inform[10]==0))   // BH+WD
//if (datafile && (inform[5]==2) && (inform[10]==1))   // BH+NS
//if (datafile && (inform[5]==1) && (inform[10]==1))   // NS+NS
//if (datafile && (inform[5]==1) && (inform[10]==2))   // NS+BH
//if (datafile && (inform[5]==2) && (inform[10]==2))   // BH+BH
//if (datafile && (inform[5]==1 && inform[10]==1)) 
//if (datafile && (inform[5]==1 && inform[10]==1)) dataout << endl << a[10] << " " << ecc; 
//{  
...
//
//}
//  if (datafile && inform[5]==1 && inform[10]==1 && a[10] < 1.0 && ecc > 0.55) 
//    {
